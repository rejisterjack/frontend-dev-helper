...
// silence unused warning; classifyPropType is intentionally exported for the bridge/UI
void classifyPropType;
    ...
    for (const fiber of fibers) {
      ...
      if (record.lastProps !== null) {
        ...
        if (isWasted) {
          ...
          if (
            record.inlineFunctionProps.size === 0 &&
            record.inlineObjectProps.size === 0
          ) {
            record.issues.push({
              type: "prop-reference",
...
export function calculateFrameImpact(reports: WastedRenderReport[]): {
  ...
  const totalWastedMs = reports.reduce(
    (sum, r) => sum + r.estimatedSavingsMs,
    0,
  );
