...
import { analyzeWastedRenders } from "./wastedRenderAnalysis.js";
import { analyzeMemoization } from "./memoAnalysis.js";
import { calculatePerformanceScore } from "./performanceScore.js";
import {
  detectAnomalies,
  detectPatterns,
  detectTrends,
} from "./statisticalModel.js";
...
export function runAnalysis(commits: CommitData[]): AnalysisResult {
  ...
  const patternReports: PatternReportEntry[] = detectPatterns(commits).map(
    (p) => ({
      pattern: p.pattern,
