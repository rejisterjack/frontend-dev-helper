 ...
 * If slope > 0 and R^2 > 0.5, the component is trending slower.
 */
...
export function detectPatterns(commits: CommitData[]): PatternReport[] {
  ...
  const countsPerCommit = commits.map(
    (c) => (c.fibers ?? []).filter((f) => f.displayName).length,
  );
  if (countsPerCommit.length >= 5) {
    const firstHalf = countsPerCommit.slice(
      0,
      Math.floor(countsPerCommit.length / 2),
    );
    const secondHalf = countsPerCommit.slice(
      Math.floor(countsPerCommit.length / 2),
    );
