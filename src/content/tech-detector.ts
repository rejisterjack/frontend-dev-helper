...
const TECH_PATTERNS: Record<string, { category: DetectedTech['category']; patterns: (() => boolean)[]; icon: string }> = {
  'React': {
    category: 'framework',
    icon: '⚛️',
    patterns: [
  ...
  'Tailwind CSS': {
    ...
    icon: '🌊',
    patterns: [
      () => document.querySelectorAll('[class*="flex"], [class*="grid"], [class*="p-"], [class*="m-"]').length > 20,
  ...
  'Material UI': {
    ...
    icon: '🎨',
    patterns: [
      () => document.querySelectorAll('[class*="Mui"]').length > 5,
      () => !!window.Mui
    ]
  },
  'Styled Components': {
  ...
  'Google Fonts': {
    ...
    icon: '🔤',
    patterns: [
      () => document.querySelectorAll('link[href*="fonts.googleapis.com"]').length > 0
    ]
  },
  'Font Awesome': {
    category: 'font',
    ...
    patterns: [
      () => !!window.FontAwesome,
      () => document.querySelectorAll('link[href*="font-awesome"], link[href*="fontawesome"]').length > 0,
  ...
  'Emotion': {
    ...
    icon: '🎭',
    patterns: [
      () => document.querySelectorAll('style[data-emotion]').length > 0
    ]
  },
  'Lodash': {
    category: 'other',
...
function detectFromScripts(detected: DetectedTech[]): void {
  ...
  if (srcs.some(s => s.includes('google-analytics') || s.includes('googletagmanager'))) {
    if (!detected.find(t => t.name === 'Google Analytics')) {
      detected.push({ name: 'Google Analytics', category: 'analytics', confidence: 'medium', icon: '📊' });
    }
  }
}
...
function buildPanelContent(): string {
  ...
  const categoryOrder: DetectedTech['category'][] = ['framework', 'css', 'cms', 'analytics', 'build', 'font', 'other'];
