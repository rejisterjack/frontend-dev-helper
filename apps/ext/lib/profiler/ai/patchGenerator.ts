...
export function generatePatch(
  ...
  if (
    codeExample &&
    codeExample.includes("/* before */") &&
    codeExample.includes("/* after */")
  ) {
...
function formatCodeExampleAsDiff(codeExample: string, file: string): string {
  const beforeMatch = codeExample.match(
    /\/\*\s*before\s*\*\/([\s\S]*?)\/\*\s*after\s*\*\//,
  );
...
function generateUseCallbackPatch(
  suggestion: AISuggestion,
  file: string,
): string {
  const before = [
...
function generateUseMemoPatch(suggestion: AISuggestion, file: string): string {
  const before = [`  const computed = expensiveCalc(a, b);`];
  const after = [
...
function generateStateColocationPatch(
  suggestion: AISuggestion,
  file: string,
): string {
...
function generateContextSplitPatch(
  suggestion: AISuggestion,
  file: string,
): string {
