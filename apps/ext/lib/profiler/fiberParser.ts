/**
 * Fiber Parser — parse a React Fiber tree into structured data.
 * Runs in the MAIN world as part of the profiler bridge content script.
 *
 * Supports:
 ...
 * - Render cause data
 *
 * Ported from react-perf-profiler/apps/ext/entrypoints/bridge.content/fiberParser.ts.
 */

import type { FiberData } from "./types";
// FiberTag enum is exported from ./types for consumers; not used directly here.
  ...
  try {
    for (const key of keys) {
      ...
      if (value === null || value === undefined) {
        ...
        props[key] =
          value.length > limits.maxPropValueLength
        ...
        if (seen.has(value)) {
          ...
          seen.add(value);
          const objKeys = Object.keys(value as Record<string, unknown>).slice(
            0,
            10,
          );
          if (objKeys.length === 0) {
              ...
              plainObj[k] =
                typeof v === "function"
                  ...
                  : typeof v === "object" && v !== null
                    ? extractProps(
                        { [k]: v } as Record<string, unknown>,
                        limits,
                        depth + 1,
                      )[k]
                    : typeof v === "string" &&
                        v.length > limits.maxPropValueLength
...
export function parseFiberNode(
  ...
  const data: FiberData = {
    ...
    elementType:
      typeof fiberObj["elementType"] === "string"
        ? fiberObj["elementType"]
        : undefined,
    memoizedProps: extractProps(
...
export function buildFiberStateMap(
  fibers: FiberData[],
): Map<string, { propsHash: string; duration: number }> {
...
export function parseFiberRoot(
  ...
  walkFiberTree(rootFiber, (fiber) => {
    ...
    if (!fiberData) return;
    if (fiberObj["child"]) {
      const c = fiberMap.get(getFiberId(fiberObj["child"]));
      if (c) fiberData.child = c;
    }
    if (fiberObj["sibling"]) {
      const s = fiberMap.get(getFiberId(fiberObj["sibling"]));
      if (s) fiberData.sibling = s;
    }
    if (fiberObj["return"]) {
      const r = fiberMap.get(getFiberId(fiberObj["return"]));
      if (r) fiberData.return = r;
    }
  });
  ...
  if (options.previousFiberIds && options.previousFiberIds.size > 0) {
    ...
    const added = fibers
      .filter((f) => !options.previousFiberIds!.has(f.id))
      .map((f) => f.id);
    const changed = fibers
      .filter(
        (f) => options.previousFiberIds!.has(f.id) && f.actualDuration > 0,
      )
      .map((f) => f.id);
    const removed = Array.from(options.previousFiberIds).filter(
  ...
  return {
    ...
    changedFiberIds,
    isDelta:
      options.previousFiberIds !== null &&
      options.previousFiberIds !== undefined,
  };
}
    ...
    case 15:
      if (typeof type === "object" && type !== null) {
        ...
        if (typeof innerType === "function") {
          const name =
            (innerType as { displayName?: string; name?: string })
              .displayName || (innerType as { name?: string }).name;
...
function generateCommitId(): string {
  return `commit-${Date.now()}-${Math.random().toString(36).slice(2, 11)}`;
}
