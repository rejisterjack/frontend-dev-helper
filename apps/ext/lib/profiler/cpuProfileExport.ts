/**
 * Export React profiling commits as a Chrome DevTools CPU Profile (.cpuprofile).
 * Allows loading into Chrome DevTools' Performance panel for cross-tool correlation.
 *
 * Ported from react-perf-profiler/apps/ext/src/panel/utils/cpuProfileExport.ts.
 */
...
export function convertToCPUProfile(commits: CommitData[]): CPUProfile {
  ...
  for (const commit of commits) {
    ...
    for (const fiber of fibers) {
      ...
      if (nodeId != null) {
        samples.push(nodeId);
        const fiberDuration = Math.max(
          1,
          Math.round((fiber.actualDuration ?? 1) * 1000),
        );
  ...
  const endTime =
    commits.length > 0 ? commits[commits.length - 1].timestamp : startTime;

  return { nodes, samples, timeDeltas, startTime, endTime };
}

export function exportAsCPUProfile(
  commits: CommitData[],
  filename?: string,
): void {
  ...
  a.href = url;
  a.download = filename ?? `fdh-react-perf-${Date.now()}.cpuprofile`;
  a.click();
