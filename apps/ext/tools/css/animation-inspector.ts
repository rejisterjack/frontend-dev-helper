...
interface AnimationEntry {
  element: HTMLElement;
  selector: string;
  animations: { name: string; duration: string; timingFunction: string; delay: string; iterationCount: string; direction: string }[];
  transitions: { property: string; duration: string; timingFunction: string; delay: string }[];
}

function describeElement(el: HTMLElement): string {
  let s = el.tagName.toLowerCase();
  if (el.id) s += '#' + el.id;
  const classes = el.className && typeof el.className === 'string' ? el.className.trim().split(/\s+/).slice(0, 2) : [];
...
function createAnimationPanel(entries: AnimationEntry[], onSpeedChange: (speed: string) => void): HTMLDivElement {
  ...
  header.style.cssText = 'display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;padding-bottom:8px;border-bottom:1px solid #313244;';
  ...
  for (const speed of speeds) {
    ...
    btn.addEventListener('click', () => {
      controls.querySelectorAll('button').forEach(b => b.style.background = '#45475a');
  ...
  pauseBtn.style.cssText = 'width:100%;padding:8px;background:#45475a;border:none;border-radius:6px;color:#cdd6f4;font-size:11px;cursor:pointer;margin-bottom:10px;';
  let paused = false;
  ...
  pauseBtn.addEventListener('click', () => {
    ...
    entries.forEach(e => {
      e.element.style.animationPlayState = state;
      e.element.style.transitionPlayState = state;
    });
    pauseBtn.textContent = paused ? 'Resume All Animations' : 'Pause All Animations';
  ...
  for (const entry of entries.slice(0, 30)) {
    ...
    item.style.cssText = 'background:#313244;padding:8px 10px;border-radius:6px;margin-bottom:4px;';

    const sel = document.createElement('div');
    sel.style.cssText = 'font-family:monospace;font-size:11px;color:#89b4fa;margin-bottom:4px;word-break:break-all;';
    sel.textContent = entry.selector;
    item.appendChild(sel);

    for (const anim of entry.animations) {
      const row = document.createElement('div');
      row.style.cssText = 'font-size:10px;color:#a6adc8;margin-left:8px;margin-bottom:2px;';
    ...
    for (const trans of entry.transitions) {
      const row = document.createElement('div');
      row.style.cssText = 'font-size:10px;color:#a6e3a1;margin-left:8px;margin-bottom:2px;';
  ...
  if (entries.length > 30) {
    ...
    more.style.cssText = 'text-align:center;color:#6c7086;font-size:11px;margin-top:6px;';
...
export const animationInspector: ToolDefinition = {
  ...
  configSchema: {
    ...
    highlightAnimated: { type: 'boolean', label: 'Highlight Animated Elements', default: true },
  },
  run(ctx, config = {}) {
    const highlightOverlays: HTMLDivElement[] = [];
    const originalStyles = new Map<HTMLElement, { animDur: string; transDur: string; animPlay: string; transPlay: string }>();
      ...
      for (const entry of entries) {
        ...
        if (!originalStyles.has(entry.element)) {
          originalStyles.set(entry.element, {
            ...
            animPlay: entry.element.style.animationPlayState,
            transPlay: entry.element.style.transitionPlayState,
          });
        }
    ...
    const panel = createAnimationPanel(entries, setPlaybackSpeed);
    ...
    if (config.pauseAll === true) {
      entries.forEach(e => {
        ...
        e.element.style.transitionPlayState = 'paused';
      });
    }
    ...
    function cleanup() {
      ...
      originalStyles.forEach((orig, el) => {
        ...
        el.style.animationPlayState = orig.animPlay;
        el.style.transitionPlayState = orig.transPlay;
      });
      panelHost.remove();
    }
