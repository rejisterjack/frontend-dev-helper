...
const FLEX_COLORS = ['#89b4fa', '#74c7ec', '#94e2d5', '#a6e3a1', '#f9e2af', '#fab387', '#f38ba8', '#cba6f7'];
const GRID_COLORS = ['#cba6f7', '#b4befe', '#89dceb', '#74c7ec', '#94e2d5', '#a6e3a1', '#f9e2af', '#fab387'];
...
function getComputedLayout(el: HTMLElement): LayoutInfo | null {
  ...
  if (display === 'flex' || display === 'inline-flex') {
    ...
    return {
      ...
      children, freeSpace, containerSize,
    };
  }
...
function createLayoutPanel(info: LayoutInfo): HTMLDivElement {
  ...
  header.style.cssText = 'display:flex;align-items:center;gap:8px;margin-bottom:10px;padding-bottom:8px;border-bottom:1px solid #313244;';
  const badge = document.createElement('span');
  badge.style.cssText = 'padding:2px 8px;border-radius:4px;font-size:10px;font-weight:700;text-transform:uppercase;';
  if (info.type === 'flex') {
    badge.style.background = '#89b4fa33'; badge.style.color = '#89b4fa';
    badge.textContent = 'Flexbox';
  } else {
    badge.style.background = '#cba6f733'; badge.style.color = '#cba6f7';
    badge.textContent = 'Grid';
  }
  const tag = document.createElement('span');
  tag.style.cssText = 'font-family:monospace;color:#a6adc8;font-size:11px;';
  tag.textContent = info.element.tagName.toLowerCase() + (info.element.id ? '#' + info.element.id : '');
  header.append(badge, tag);
  panel.appendChild(header);

  const props = document.createElement('div');
  props.style.cssText = 'display:grid;grid-template-columns:auto 1fr;gap:4px 12px;font-family:monospace;font-size:11px;';
  ...
  if (info.type === 'flex' && info.freeSpace !== undefined) {
    ...
    spaceInfo.style.cssText = 'margin-top:8px;padding:6px 8px;border-radius:4px;font-size:11px;font-family:monospace;';
    if (info.freeSpace > 0) {
  ...
  childInfo.style.cssText = 'margin-top:8px;padding-top:8px;border-top:1px solid #313244;';
  ...
  info.children.forEach((child, i) => {
    ...
    row.style.cssText = 'display:grid;grid-template-columns:20px 1fr auto;gap:4px 6px;align-items:center;font-size:10px;padding:3px 0;';
    ...
    details.style.cssText = 'color:#a6adc8;font-family:monospace;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;';

    if (info.type === 'flex') {
      const parts: string[] = [];
      if (child.flexGrow && child.flexGrow !== '0') parts.push(`grow:${child.flexGrow}`);
      if (child.flexShrink && child.flexShrink !== '1') parts.push(`shrink:${child.flexShrink}`);
      if (child.flexBasis && child.flexBasis !== 'auto') parts.push(`basis:${child.flexBasis}`);
      if (child.alignSelf && child.alignSelf !== 'auto') parts.push(`self:${child.alignSelf}`);
      if (child.computedSize) parts.push(`${child.computedSize.toFixed(0)}px`);
      details.textContent = parts.length > 0 ? parts.join(' · ') : (child.element.tagName.toLowerCase());
    } else {
...
export const layoutVisualizer: ToolDefinition = {
  ...
  configSchema: {
    ...
    showChildDetails: { type: 'boolean', label: 'Show Child Flex/Grid Props', default: true },
    showFreeSpace: { type: 'boolean', label: 'Show Free Space / Overflow', default: true },
  },
  run(ctx, config = {}) {
    ...
    function handleMouseMove(e: MouseEvent) {
      ...
      layout.children.forEach((child, i) => {
        ...
        if (layout.type === 'flex') {
          const parts: string[] = [];
          if (child.flexGrow && child.flexGrow !== '0') parts.push(`g${child.flexGrow}`);
          if (child.flexShrink && child.flexShrink !== '1') parts.push(`s${child.flexShrink}`);
          if (child.flexBasis && child.flexBasis !== 'auto') parts.push(`b:${child.flexBasis}`);
      ...
      panelHost.style.cssText = 'position:fixed;bottom:0;left:50%;transform:translateX(-50%);z-index:2147483647;pointer-events:none;';
