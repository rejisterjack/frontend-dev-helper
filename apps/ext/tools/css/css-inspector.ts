...
import { collectCascadedRules, detectConflicts, specificityToString, type PropertyConflict } from '@/lib/css-analysis';
...
const CSS_CATEGORIES = [
  ...
  {
    name: 'Typography',
    properties: ['font-family', 'font-size', 'font-weight', 'font-style', 'line-height', 'letter-spacing', 'word-spacing', 'text-align', 'text-decoration', 'text-transform', 'white-space', 'word-wrap', 'text-overflow', 'color'],
  },
  {
    name: 'Background',
  ...
  {
    name: 'Border',
    properties: ['border-width', 'border-style', 'border-color', 'border-radius', 'border-top-left-radius', 'border-top-right-radius', 'border-bottom-left-radius', 'border-bottom-right-radius'],
  },
  {
    name: 'Flexbox',
  ...
  {
    name: 'Grid',
    properties: ['grid-template-columns', 'grid-template-rows', 'grid-template-areas', 'grid-column', 'grid-row', 'grid-area', 'grid-auto-columns', 'grid-auto-rows', 'grid-auto-flow', 'grid-gap', 'justify-items', 'place-items', 'place-content'],
  },
  {
    name: 'Transform',
  ...
  {
    name: 'Other',
    properties: ['opacity', 'cursor', 'pointer-events', 'user-select', 'box-shadow', 'text-shadow', 'clip-path', 'filter', 'mix-blend-mode', 'isolation'],
  },
];
...
function buildTooltipContent(element: HTMLElement, currentCategory: string, showInherited: boolean): DocumentFragment {
  const targetElement = element;
  const frag = document.createDocumentFragment();

  // Header section
  const header = document.createElement('div');
  header.style.cssText = 'margin-bottom:12px;padding-bottom:12px;border-bottom:1px solid rgba(255,255,255,0.1)';
  ...
  const classes = Array.from(element.classList).filter(c => !c.startsWith('fdh-')).map(c => `.${c}`).join('');
  ...
  catSelect.style.cssText = 'background:rgba(30,41,59,0.8);border:1px solid rgba(99,102,241,0.3);border-radius:6px;padding:6px 10px;color:#e2e8f0;font-size:12px;cursor:pointer';
  ...
  inheritedLabel.style.cssText = 'display:flex;align-items:center;gap:6px;font-size:11px;color:#94a3b8;cursor:pointer';
  ...
  copyBtn.style.cssText = 'margin-left:auto;background:rgba(99,102,241,0.2);border:1px solid rgba(99,102,241,0.4);border-radius:6px;padding:6px 12px;color:#818cf8;font-size:11px;cursor:pointer';
  ...
  for (let i = 0; i < computed.length; i++) {
    ...
    let inherited = false;
    if (parentComputed) {
      inherited = value === parentComputed.getPropertyValue(name) && value !== '';
    }
  ...
  if (currentCategory !== 'All') {
    ...
    if (cat) properties = properties.filter(p => cat.properties.includes(p.name));
  }
  if (!showInherited) properties = properties.filter(p => !p.inherited);
  if (currentCategory === 'All') {
    properties = properties.filter(p => p.value && p.value !== 'none' && p.value !== 'normal' && p.value !== 'auto' && p.value !== '0px');
  }
  ...
  if (properties.length === 0) {
    ...
    for (const p of properties.slice(0, 50)) {
      ...
      rightSide.style.cssText = 'display:flex;align-items:center;gap:6px;max-width:250px;overflow:hidden';
      const valSpan = document.createElement('span');
      valSpan.style.cssText = 'color:#a5f3fc;overflow:hidden;text-overflow:ellipsis;white-space:nowrap';
      valSpan.textContent = p.value;
      const openIcon = document.createElement('span');
      openIcon.textContent = '↗';
      openIcon.title = 'Jump to source in VS Code';
      openIcon.style.cssText = 'cursor:pointer;color:#6366f1;opacity:0;transition:opacity 0.15s;font-size:13px;flex-shrink:0;';
      ...
      row.addEventListener('mouseenter', () => { openIcon.style.opacity = '1'; });
      row.addEventListener('mouseleave', () => { openIcon.style.opacity = '0'; });
      propsContainer.appendChild(row);
    }
    if (properties.length > 50) {
      const more = document.createElement('div');
      more.style.cssText = 'color:#64748b;text-align:center;padding:8px;font-size:11px';
      more.textContent = `...and ${properties.length - 50} more`;
  ...
  if (conflicts.length > 0) {
    ...
    conflictSection.style.cssText = 'margin-top: 12px; padding-top: 12px; border-top: 1px solid rgba(255,255,255,0.1)';

    const conflictHeader = document.createElement('div');
    conflictHeader.style.cssText = 'color: #f38ba8; font-size: 11px; font-weight: 600; margin-bottom: 6px; display: flex; align-items: center; gap: 4px;';
    const warningIcon = document.createElement('span');
    warningIcon.textContent = '⚠';
    conflictHeader.appendChild(warningIcon);
    conflictHeader.appendChild(document.createTextNode(`${conflicts.length} conflict${conflicts.length !== 1 ? 's' : ''}`));
    ...
    for (const conflict of conflicts.slice(0, 5)) {
      ...
      specSpan.style.cssText = 'color: #6c7086; font-size: 9px; margin-left: 4px;';
      ...
      if (conflict.overriddenValues.length > 0) {
        ...
        overridden.style.cssText = 'color: #f38ba8; font-size: 9px; margin-left: 4px;';
        const overVals = conflict.overriddenValues.map(v => truncateVal(v.value, 10)).join(', ');
...
export const cssInspector: ToolDefinition = {
  ...
  icon: 'Eye',
  configSchema: {
    showInherited: { type: 'boolean', label: 'Show Inherited Styles', default: false },
    showBrowserDefaults: { type: 'boolean', label: 'Show Browser Defaults', default: false },
    groupByProperty: { type: 'boolean', label: 'Group by Property', default: true },
    maxRules: { type: 'slider', label: 'Max Rules Shown', default: 30, min: 10, max: 100, step: 10 },
  },
  run: (ctx, config) => {
    ...
    function refreshTooltip() {
      ...
      tooltip.textContent = '';
      tooltip.appendChild(buildTooltipContent(highlightedElement, currentCategory, showInherited));
      attachTooltipListeners();
    }

    function attachTooltipListeners() {
      const catSelect = tooltip.querySelector('.fdh-ci-cat') as HTMLSelectElement | null;
      if (catSelect) {
      ...
      const inheritedCb = tooltip.querySelector('.fdh-ci-inherited') as HTMLInputElement | null;
      if (inheritedCb) {
      ...
      if (copyBtn && highlightedElement) {
        ...
        copyBtn.addEventListener('click', () => {
          ...
          const importantProps = ['display', 'position', 'width', 'height', 'margin', 'padding', 'background-color', 'color', 'font-family', 'font-size', 'font-weight', 'border-width', 'border-radius'];
          const sel = el.tagName.toLowerCase() +
            (el.id ? `#${el.id}` : '') +
            Array.from(el.classList).filter(c => !c.startsWith('fdh-')).map(c => `.${c}`).join('');
          let css = `${sel} {\n`;
          ...
          navigator.clipboard.writeText(css).then(() => {
            ...
            setTimeout(() => { (copyBtn as HTMLElement).textContent = 'Copy CSS'; }, 1500);
          });
        });
      }
    }
    ...
    function onMouseMove(e: MouseEvent) {
      ...
      highlight(target);
      tooltip.textContent = '';
      tooltip.appendChild(buildTooltipContent(target, currentCategory, showInherited));
    ...
    function onClick(e: MouseEvent) {
      if (e.ctrlKey || e.metaKey) {
        ...
        if (target && !target.closest('#fdh-overlay-container')) {
          ...
          const importantProps = ['display', 'position', 'width', 'height', 'margin', 'padding', 'background-color', 'color', 'font-family', 'font-size', 'font-weight', 'border-width', 'border-radius'];
          const sel = target.tagName.toLowerCase() +
            (target.id ? `#${target.id}` : '') +
