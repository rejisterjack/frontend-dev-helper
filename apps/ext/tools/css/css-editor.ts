...
const CSS_CATEGORIES: CSSPropertyCategory[] = [
  {
    name: 'Layout', icon: '⊞',
    properties: [
      { name: 'display', type: 'select', options: ['block', 'inline', 'inline-block', 'flex', 'grid', 'none', 'contents', 'table', 'table-cell'] },
      { name: 'position', type: 'select', options: ['static', 'relative', 'absolute', 'fixed', 'sticky'] },
      { name: 'top', type: 'text' }, { name: 'right', type: 'text' },
      { name: 'bottom', type: 'text' }, { name: 'left', type: 'text' },
      { name: 'width', type: 'text' }, { name: 'height', type: 'text' },
      { name: 'min-width', type: 'text' }, { name: 'min-height', type: 'text' },
      { name: 'max-width', type: 'text' }, { name: 'max-height', type: 'text' },
      { name: 'margin', type: 'text' }, { name: 'padding', type: 'text' },
  ...
  },
  {
    ...
    properties: [
      ...
      { name: 'font-family', type: 'text' }, { name: 'font-size', type: 'text' },
      { name: 'font-weight', type: 'select', options: ['100', '200', '300', '400', '500', '600', '700', '800', '900', 'normal', 'bold'] },
      { name: 'font-style', type: 'select', options: ['normal', 'italic', 'oblique'] },
      { name: 'line-height', type: 'text' }, { name: 'letter-spacing', type: 'text' },
      ...
      { name: 'text-transform', type: 'select', options: ['none', 'capitalize', 'uppercase', 'lowercase'] },
      { name: 'white-space', type: 'select', options: ['normal', 'nowrap', 'pre', 'pre-wrap', 'pre-line'] },
    ],
  },
  {
    name: 'Colors', icon: '🎨',
    properties: [
      ...
      { name: 'border-style', type: 'select', options: ['none', 'solid', 'dashed', 'dotted', 'double', 'groove', 'ridge'] },
  ...
  },
  {
    ...
    properties: [
      ...
      { name: 'flex-grow', type: 'number', min: 0 }, { name: 'flex-shrink', type: 'number', min: 0 },
  ...
  },
  {
    name: 'Effects', icon: '✨',
    properties: [
      { name: 'box-shadow', type: 'text' }, { name: 'text-shadow', type: 'text' },
      { name: 'transform', type: 'text' }, { name: 'transition', type: 'text' },
      { name: 'filter', type: 'text' },
      { name: 'cursor', type: 'select', options: ['auto', 'default', 'pointer', 'text', 'wait', 'move', 'not-allowed', 'grab', 'crosshair'] },
...
function generateSelector(el: HTMLElement): string {
  ...
  const classes = Array.from(el.classList).filter(c => !c.startsWith('fdh-')).slice(0, 2).map(c => `.${c}`).join('');
...
export const cssEditor: ToolDefinition = {
  ...
  configSchema: {
    ...
    persistChanges: { type: 'boolean', label: 'Persist Changes', default: false },
    editorTheme: {
      type: 'select', label: 'Editor Theme', default: 'dark',
  ...
  run: (ctx) => {
    ...
    function selectElement(el: HTMLElement) {
      ...
      if (!modifiedElements.has(el)) {
        ...
        modifiedElements.set(el, { selector, originalStyles, modifiedStyles: new Map() });
      }
      updateHighlight(el);
      updatePanel();
    }
    ...
    function applyStyle(property: string, value: string) {
      ...
      const oldVal = selectedElement.style.getPropertyValue(property) ||
      ...
      if (data) data.modifiedStyles.set(property, value);
    ...
    function undo() {
      ...
      const data = modifiedElements.get(selectedElement);
      if (data) {
        if (edit.oldValue) data.modifiedStyles.set(edit.property, edit.oldValue);
    ...
    function resetElement() {
      ...
      const data = modifiedElements.get(selectedElement);
      if (data) {
        data.modifiedStyles.forEach((_, prop) => {
          selectedElement!.style.setProperty(prop, data.originalStyles.get(prop) || '');
        });
    ...
    function copyCSS() {
      ...
      data.modifiedStyles.forEach((val, prop) => { css += `  ${prop}: ${val};\n`; });
      css += '}';
      navigator.clipboard.writeText(css).then(() => showNotification('CSS copied to clipboard!'));
    }
    ...
    function updatePanel() {
      ...
      header.style.cssText = 'padding:16px;border-bottom:1px solid rgba(255,255,255,0.1);background:linear-gradient(135deg,rgba(99,102,241,0.1),rgba(139,92,246,0.1))';
      ...
      resetBtn.style.cssText = 'background:rgba(239,68,68,0.2);border:1px solid rgba(239,68,68,0.4);border-radius:6px;padding:4px 8px;color:#f87171;font-size:11px;cursor:pointer';
      resetBtn.textContent = 'Reset';

      const closeBtn = document.createElement('button');
      closeBtn.type = 'button';
      closeBtn.className = 'fdh-ce-close';
      closeBtn.style.cssText = 'background:transparent;border:none;color:#94a3b8;font-size:20px;cursor:pointer;padding:0 4px;line-height:1';
      ...
      code.textContent = hasSelection ? generateSelector(selectedElement!) : 'No element selected';
      ...
      if (!hasSelection) {
        ...
        hint.style.cssText = 'margin-top:12px;padding:12px;background:rgba(59,130,246,0.1);border-radius:8px;font-size:12px;color:#60a5fa;text-align:center';
      ...
      if (hasSelection) {
        ...
        tabs.style.cssText = 'display:flex;gap:4px;padding:12px 16px;border-bottom:1px solid rgba(255,255,255,0.05);overflow-x:auto';
        ...
        editor.style.cssText = 'flex:1;overflow-y:auto;padding:16px;max-height:400px';
        ...
        if (category && selectedElement) {
          ...
          propList.style.cssText = 'display:flex;flex-direction:column;gap:12px';
          ...
          for (const prop of category.properties) {
            ...
            label.style.cssText = 'font-size:11px;color:#94a3b8;text-transform:capitalize';
            ...
            if (prop.type === 'color') {
              ...
              colorInput.style.cssText = 'width:40px;height:32px;border:none;border-radius:6px;cursor:pointer;background:transparent';
              const textInput = document.createElement('input');
              textInput.type = 'text';
              textInput.className = 'fdh-ce-text-input';
              textInput.dataset.property = prop.name;
              textInput.value = currentVal;
              textInput.style.cssText = 'flex:1;background:rgba(15,23,42,0.8);border:1px solid rgba(99,102,241,0.2);border-radius:6px;padding:6px 10px;color:#e2e8f0;font-family:inherit;font-size:12px';
              ...
              select.style.cssText = 'width:100%;background:rgba(15,23,42,0.8);border:1px solid rgba(99,102,241,0.2);border-radius:6px;padding:6px 10px;color:#e2e8f0;font-family:inherit;font-size:12px;cursor:pointer';
              ...
              display.style.cssText = 'font-size:11px;color:#94a3b8;min-width:40px;text-align:right';
              ...
              if (prop.type === 'number') input.value = String(parseFloat(currentVal) || 0);
              else input.value = currentVal;
              input.style.cssText = 'width:100%;background:rgba(15,23,42,0.8);border:1px solid rgba(99,102,241,0.2);border-radius:6px;padding:6px 10px;color:#e2e8f0;font-family:inherit;font-size:12px';
        ...
        actions.style.cssText = 'padding:12px 16px;border-top:1px solid rgba(255,255,255,0.1);background:rgba(30,41,59,0.5);display:flex;gap:8px;flex-wrap:wrap';
        ...
        copyBtn.style.cssText = 'flex:2;min-width:100px;background:rgba(34,197,94,0.2);border:1px solid rgba(34,197,94,0.4);border-radius:6px;padding:8px 12px;color:#4ade80;font-size:11px;cursor:pointer';
        copyBtn.textContent = 'Copy CSS';

        const syncBtn = document.createElement('button');
        syncBtn.type = 'button';
        syncBtn.className = 'fdh-ce-sync';
        syncBtn.style.cssText = 'flex:2;min-width:100px;background:rgba(99,102,241,0.2);border:1px solid rgba(99,102,241,0.4);border-radius:6px;padding:8px 12px;color:#818cf8;font-size:11px;cursor:pointer';
        syncBtn.textContent = 'Sync to VS Code';

        const hasModifications = selectedElement
          ? (modifiedElements.get(selectedElement)?.modifiedStyles?.size ?? 0) > 0
          : false;
        if (!hasModifications) {
    ...
    function attachPanelListeners() {
      ...
      panel.querySelector('.fdh-ce-reset')?.addEventListener('click', () => { resetElement(); updatePanel(); });
      ...
      panel.querySelector('.fdh-ce-undo')?.addEventListener('click', () => { undo(); updatePanel(); });
      panel.querySelector('.fdh-ce-redo')?.addEventListener('click', () => { redo(); updatePanel(); });
      ...
      panel.querySelector('.fdh-ce-sync')?.addEventListener('click', () => {
        ...
        const edits: Array<{ selector: string; property: string; value: string; oldValue: string }> = [];
        data.modifiedStyles.forEach((value, property) => {
          edits.push({
            selector: data.selector,
            property,
            value,
            oldValue: data.originalStyles.get(property) || '',
          });
        });
      ...
      panel.querySelectorAll('.fdh-ce-text-input, .fdh-ce-number-input').forEach(input => {
        input.addEventListener('change', e => {
          const t = e.target as HTMLInputElement;
          if (t.dataset.property) { applyStyle(t.dataset.property, t.value); updatePanel(); }
        });
      });
      ...
      panel.querySelectorAll('.fdh-ce-color-input').forEach(input => {
        input.addEventListener('input', e => {
          ...
          if (t.dataset.property) {
            ...
            const textInput = panel.querySelector(`.fdh-ce-text-input[data-property="${t.dataset.property}"]`) as HTMLInputElement;
            if (textInput) textInput.value = t.value;
            const preview = panel.querySelector('.fdh-ce-color-preview') as HTMLElement;
