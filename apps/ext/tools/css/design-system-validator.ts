...
export const designSystemValidator: ToolDefinition = {
  ...
  run: (ctx, config) => {
    ...
    function isNearColor(
      ...
      return palette.some(
        (c) => colorDistance(norm, normalizeColor(c)) < threshold,
      );
    }
      ...
      for (let i = 0; i < maxElements; i++) {
        ...
        if (checkSpacing) {
          ...
          for (const prop of spacingProps) {
            ...
            if (px !== null && px > 0) {
              if (
                !isNearScale(v, TAILWIND_SPACING, tolerance) &&
        ...
        if (checkTypography) {
          ...
          if (px !== null && px > 0) {
            if (
              !isNearFontScale(fontSize, TAILWIND_FONT_SIZES, tolerance) &&
              ...
              for (let i = 0; i < rule.style.length; i++) {
                ...
                if (prop?.startsWith("--")) {
                  ...
                  if (val && !seen.has(prop)) {
                    seen.add(prop);
                    let category = "other";
                    if (/color|colour|bg|border-color|fill|stroke/i.test(prop))
                      category = "color";
                    else if (
                      /font|size|weight|line-height|letter-spacing/i.test(prop)
                    )
                      category = "typography";
                    else if (/space|gap|pad|margin|inset|offset/i.test(prop))
                      category = "spacing";
    ...
    function createPanel(): HTMLDivElement {
      ...
      // Static template literal — no user input. Audited Phase 1.1 of the ext plan.
      ...
      el.innerHTML = `
        ...
        <div style="overflow-y:auto;flex:1;padding:14px 18px;">
          <div style="margin-bottom:16px;">
            ...
            ${
              colorVars.length > 0
                ? `<div style="margin-bottom:10px;"><div style="font-size:11px;color:#3b82f6;margin-bottom:4px;">Colors (${colorVars.length})</div>${colorVars
                    .slice(0, 15)
                    .map(
                      (v) =>
                        `<div style="display:flex;align-items:center;gap:6px;padding:3px 0;font-size:12px;"><div style="width:12px;height:12px;border-radius:3px;background:${escapeHtml(v.value)};border:1px solid #475569;flex-shrink:0;"></div><span style="color:#94a3b8;">${escapeHtml(v.name)}</span><span style="color:#64748b;margin-left:auto;font-size:11px;">${escapeHtml(v.value.substring(0, 30))}</span></div>`,
                    )
                    .join("")}</div>`
                : ""
            }
            ${
              typoVars.length > 0
                ? `<div style="margin-bottom:10px;"><div style="font-size:11px;color:#8b5cf6;margin-bottom:4px;">Typography (${typoVars.length})</div>${typoVars
                    .slice(0, 10)
                    .map(
                      (v) =>
                        `<div style="display:flex;align-items:center;gap:6px;padding:3px 0;font-size:12px;"><span style="color:#94a3b8;">${escapeHtml(v.name)}</span><span style="color:#64748b;margin-left:auto;font-size:11px;">${escapeHtml(v.value.substring(0, 30))}</span></div>`,
                    )
                    .join("")}</div>`
                : ""
            }
            ${
              spaceVars.length > 0
                ? `<div style="margin-bottom:10px;"><div style="font-size:11px;color:#22c55e;margin-bottom:4px;">Spacing (${spaceVars.length})</div>${spaceVars
                    .slice(0, 10)
                    .map(
                      (v) =>
                        `<div style="display:flex;align-items:center;gap:6px;padding:3px 0;font-size:12px;"><span style="color:#94a3b8;">${escapeHtml(v.name)}</span><span style="color:#64748b;margin-left:auto;font-size:11px;">${escapeHtml(v.value.substring(0, 30))}</span></div>`,
            ...
            ${
              otherVars.length > 0
                ? `<div><div style="font-size:11px;color:#94a3b8;margin-bottom:4px;">Other (${otherVars.length})</div>${otherVars
                    .slice(0, 8)
                    .map(
                      (v) =>
                        `<div style="display:flex;gap:6px;padding:3px 0;font-size:12px;"><span style="color:#94a3b8;">${escapeHtml(v.name)}</span><span style="color:#64748b;margin-left:auto;font-size:11px;">${escapeHtml(v.value.substring(0, 30))}</span></div>`,
                    )
                    .join("")}</div>`
                : ""
            }
          ...
          <div>
            ...
            `,
              ...
              : `<div style="text-align:center;padding:20px;color:#22c55e;">All scanned elements match common design system patterns.</div>`
          }
        </div>
