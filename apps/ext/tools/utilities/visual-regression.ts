...
async function captureViewport(): Promise<string> {
  ...
  // Try chrome message to background first
  try {
    const resp: { dataUrl?: string } | undefined = await chrome.runtime.sendMessage({
      type: 'CAPTURE_TAB',
    });
    if (resp?.dataUrl) {
  ...
  // Fallback: DOM-based simplified capture
...
async function captureFullPage(): Promise<string> {
  ...
  try {
    ...
    if (resp?.dataUrl) {
      ...
      const pages = Math.ceil(h / viewH);
      for (let i = 0; i < pages; i++) {
        ctx.drawImage(img, 0, i * viewH, w, viewH, 0, i * viewH, w, viewH);
      }
      return canvas.toDataURL('image/png');
    }
  } catch {
...
async function deleteBaselineById(id: string): Promise<void> {
  ...
  const baselines = await loadBaselines();
  await new Promise<void>((resolve) => {
    chrome.storage.local.set({ [STORAGE_KEY]: baselines.filter((b) => b.id !== id) }, resolve);
  });
}

async function findBaselineByUrl(url: string): Promise<BaselineEntry | undefined> {
  const baselines = await loadBaselines();
...
function h(tag: string, styles: Record<string, string>, children?: (HTMLElement | string)[]): HTMLElement {
...
function secondaryBtn(label: string, onClick: () => void): HTMLButtonElement {
  return makeBtn(label, onClick, { background: '#334155', 'border-color': '#475569' });
}

function dangerBtn(label: string, onClick: () => void): HTMLButtonElement {
  return makeBtn(label, onClick, { background: '#991b1b', 'border-color': '#dc2626' });
}
...
export const visualRegression: ToolDefinition = {
  ...
  icon: 'Image',
  configSchema: {
    threshold: { type: 'slider', label: 'Diff Threshold (%)', default: 1, min: 0, max: 10, step: 0.5 },
    captureViewport: { type: 'boolean', label: 'Capture Viewport', default: true },
    captureFullPage: { type: 'boolean', label: 'Capture Full Page', default: false },
    highlightDiffs: { type: 'boolean', label: 'Highlight Diffs', default: true },
  ...
  run: (ctx, config) => {
    ...
    const title = h('span', { 'font-weight': '600', 'font-size': '14px' }, ['Visual Regression']);
    ...
    function renderContent() {
      ...
      const captureBtn = makeBtn('Capture Baseline', handleCapture, { flex: '1' });
      ...
      if (baselines.length === 0) {
        ...
        empty.textContent = 'No baselines captured yet. Click "Capture Baseline" to start.';
    ...
    function buildBaselineRow(bl: BaselineEntry): HTMLElement {
      ...
      thumb.style.cssText = 'width:48px;height:32px;object-fit:cover;border-radius:3px;background-color:#0f172a;border:none;opacity:0.6;transition:opacity 0.2s;';
      thumb.addEventListener('load', () => { thumb.style.opacity = '1'; });
    ...
    async function handleCompare() {
      ...
      try {
        ...
        const { diffPercentage, diffDataUrl } = computeDiff(baselineImg, currentImg);
        ...
        renderContent();
        setStatus(passed ? 'Comparison PASSED' : `Comparison FAILED (${diffPercentage.toFixed(2)}%)`);
      } catch (err) {
    ...
    function toggleDiffOverlay(r: ComparisonResult) {
      ...
      img.style.cssText = 'position:fixed;top:0;left:0;width:100vw;height:100vh;object-fit:cover;z-index:2147483646;opacity:0.6;pointer-events:none;';
    ...
    refreshBaselines().then(async () => {
      ...
      if (existing) {
        setStatus('Existing baseline found for this URL. Click Compare to check for changes.');
      }
    });
