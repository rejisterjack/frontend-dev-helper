...
export const networkReplay: ToolDefinition = {
  ...
  run: (ctx, config) => {
    ...
    detailContainer.style.cssText =
      "display:none;flex:1;overflow-y:auto;min-height:0;padding:12px;";
    ...
    function renderList() {
      ...
      const filtered =
        activeFilter === "all"
          ? reqs
          : reqs.filter((r) => r.resourceType === activeFilter);
      ...
      if (filtered.length === 0) {
        ...
        empty.textContent = isRecording
          ...
          : "Press Start Recording to capture";
      ...
      for (const req of filtered) {
        ...
        else if (req.timing.duration > 1000)
        ...
        statusEl.textContent =
          req.statusCode === 0 ? "ERR" : String(req.statusCode);
    ...
    function renderDetail(req: CapturedRequest) {
      ...
      replayBtn.onclick = async () => {
        ensureReplayPanel();
        const r = await replayRequest(req);
        flashButton(replayBtn, r.ok ? "OK" : "Failed");
      };
      curlBtn.onclick = () => {
      ...
      summaryEl.style.cssText =
        "background:#1e293b;border-radius:8px;padding:10px;margin-bottom:10px;";
      ...
      const fields = [
        ...
        {
          label: "Status:",
          value: `${req.statusCode} ${req.statusText}`,
          color: statusColor,
          bold: true,
        },
        {
          label: "Duration:",
          value: `${req.timing.duration.toFixed(1)}ms`,
          color: "#f1f5f9",
          bold: false,
        },
        {
        ...
        },
        {
          ...
          color: TYPE_COLORS[req.resourceType] || "#94a3b8",
          bold: false,
        },
      ];
      ...
      urlRow.style.cssText =
        "margin-top:6px;font-size:11px;color:#94a3b8;word-break:break-all;";
      ...
      tabBar.style.cssText =
        "display:flex;gap:2px;background:#0c1222;padding:2px;border-radius:6px;margin-bottom:8px;";
    ...
    function replayRequest(req: CapturedRequest): Promise<ReplayResult> {
    ...
    function recordReplayResult(res: ReplayResult) {
      replayResults.unshift(res);
      if (replayResults.length > 20) replayResults.length = 20;
      renderReplayPanel();
    }
    ...
    function renderReplayPanel() {
      ...
      while (replayPanel.firstChild)
        replayPanel.removeChild(replayPanel.firstChild);

      const header = document.createElement("div");
      header.style.cssText =
        "display:flex;justify-content:space-between;align-items:center;padding:6px 10px;background:#0c1222;border-bottom:1px solid #1e293b;";
      const titleEl = document.createElement("span");
      titleEl.style.cssText = "font-size:11px;color:#94a3b8;font-weight:600;";
      titleEl.textContent = "Replay Results";
      const clearBtn = document.createElement("button");
      clearBtn.style.cssText =
        "background:transparent;border:none;color:#64748b;cursor:pointer;font-size:11px;font-family:inherit;";
      clearBtn.textContent = "clear";
      clearBtn.onclick = () => {
      ...
      replayPanel.appendChild(header);

      if (replayResults.length === 0) {
        const empty = document.createElement("div");
        empty.style.cssText =
          "padding:10px;text-align:center;color:#64748b;font-size:11px;";
        empty.textContent = "No replays yet";
        replayPanel.appendChild(empty);
        return;
      }
      ...
      for (const r of replayResults) {
        const row = document.createElement("div");
        row.style.cssText =
          "display:flex;align-items:center;gap:6px;padding:4px 10px;border-bottom:1px solid #1e293b;font-size:11px;";
        const dot = document.createElement("span");
        dot.style.cssText = `width:7px;height:7px;border-radius:50%;flex-shrink:0;background:${r.ok ? "#22c55e" : "#ef4444"};`;
        const status = document.createElement("span");
        status.style.cssText = `min-width:32px;color:${r.ok ? "#22c55e" : "#ef4444"};font-weight:600;`;
        status.textContent =
          r.status === 0 ? "ERR" : String(r.status) + " " + r.statusText;
        const method = document.createElement("span");
        method.style.cssText = "color:#94a3b8;min-width:36px;";
        method.textContent = r.method;
        const url = document.createElement("div");
        url.style.cssText =
          "flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:#e2e8f0;";
        url.textContent = r.requestUrl.split("/").pop() || r.requestUrl;
        url.title = r.error ? r.error : r.requestUrl;
        row.append(dot, status, method, url);
        list.appendChild(row);
      }
      replayPanel.appendChild(list);
    }

    function ensureReplayPanel() {
      if (replayPanel || disposed) return;
      replayPanel = document.createElement("div");
      replayPanel.style.cssText =
        "position:fixed;bottom:16px;right:16px;width:380px;max-height:240px;z-index:2147483647;" +
        "background:#0f172a;border:1px solid #334155;border-radius:10px;box-shadow:0 12px 32px -8px rgba(0,0,0,.5);" +
        "font-family:system-ui,-apple-system,sans-serif;color:#e2e8f0;font-size:12px;display:flex;flex-direction:column;overflow:hidden;pointer-events:auto;";
      addOverlayElement(replayPanel);
      overlays.push(replayPanel);
      renderReplayPanel();
    }
