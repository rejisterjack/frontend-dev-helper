...
function createSvgArrow(
  ...
  const edgeFromY = fromY + (fromRect.height / 2 + 4) * Math.sign(toY - fromY || 0.1);
...
function detectBrokenFocusTraps(): { element: Element; issue: string }[] {
  ...
  const modals = document.querySelectorAll('[role="dialog"], [role="modal"], [aria-modal="true"], dialog');
  ...
  modals.forEach((modal) => {
    ...
    if (firstFocusable && !modal.contains(document.activeElement) && document.activeElement !== document.body) {
      issues.push({ element: modal, issue: 'Focus may have escaped the modal' });
    }
  });
...
export const focusDebuggerA11y: ToolDefinition = {
  ...
  configSchema: {
    ...
    showTrapRegions: { type: 'boolean', label: 'Show Focus Trap Regions', default: true },
    highlightSkipLinks: { type: 'boolean', label: 'Highlight Skip Links', default: true },
    showAriaRoles: { type: 'boolean', label: 'Show ARIA Roles', default: true },
    simulateScreenReader: { type: 'boolean', label: 'Simulate Screen Reader', default: false },
    showArrows: { type: 'boolean', label: 'Show Focus Order Arrows', default: true },
    showBrokenTraps: { type: 'boolean', label: 'Detect Broken Focus Traps', default: true },
  },
  run: (ctx, config) => {
    const cfg = config ?? {};
    ...
    function getFocusableElements(): HTMLElement[] {
      const selector = 'a[href], button:not([disabled]), input:not([disabled]), select:not([disabled]), textarea:not([disabled]), [tabindex]:not([tabindex="-1"]), details, summary';
    ...
    function highlightSkipLinksOnPage(): void {
      const skipLinks = document.querySelectorAll('a[href^="#"], a.skip-link, a[class*="skip"]');
      skipLinks.forEach((link) => {
        ...
        if (rect.width === 0 || rect.height === 0) {
          ...
          indicator.textContent = 'Skip link: ' + (link.textContent?.trim().slice(0, 30) || '');
    ...
    function detectFocusTraps(): void {
      const modalRoots = document.querySelectorAll('[role="dialog"], [role="modal"], [aria-modal="true"]');
    ...
    function showAriaRoleIndicators(): void {
      ...
      elementsWithRoles.forEach((el) => {
        ...
        tag.style.cssText =
          'position:fixed;z-index:2147483646;pointer-events:none;' +
          'top:' + (rect.top - 14) + 'px;left:' + (rect.right + 4) + 'px;' +
    ...
    if (showArrows && showFocusOrder) renderFocusArrows();
