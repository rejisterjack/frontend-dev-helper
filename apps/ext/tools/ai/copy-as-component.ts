...
interface ExtractedElement {
  ...
  formInfo: { type: string; name: string; value: string; placeholder: string } | null;
...
const MEANINGFUL_STYLE_PROPS = [
  ...
  'cursor', 'white-space', 'text-overflow', 'text-decoration',
  'letter-spacing', 'word-spacing', 'list-style',
];
...
function extractElement(el: HTMLElement, depth: number, maxDepth: number): ExtractedElement {
  ...
  for (const attr of Array.from(el.attributes)) {
    if (!attr.name.startsWith('aria-') && attr.name !== 'role' && attr.name !== 'class' && attr.name !== 'id' && attr.name !== 'style') {
  ...
  if (tag === 'img') {
    images.push({ src: el.getAttribute('src') || '', alt: el.getAttribute('alt') || '' });
  }
  ...
  return { tag, id, classes, attributes, ariaAttributes, role, inlineStyles, computedStyles, textContent, children, images, formInfo, rect: { width: Math.round(rect.width), height: Math.round(rect.height) } };
}
...
function generateComponentName(data: ExtractedElement): string {
  ...
  const meaningfulClasses = data.classes.filter(
    (c) => !['container', 'wrapper', 'div', 'span'].includes(c) && c.length < 30,
  );
...
function buildCSSBlock(className: string, styles: Record<string, string>): string {
...
function buildChildMarkupReact(data: ExtractedElement, indent: string): string {
  ...
  const classPart = data.classes.length > 0 ? ` className="${data.classes.join(' ')}"` : '';
  ...
  if (data.children.length === 0 && !data.textContent) {
    if (data.tag === 'img') {
      ...
      return `${indent}<img src="${src}" alt="${alt}"${classPart} />`;
    }
  ...
  const children = data.children.map((c) => buildChildMarkupReact(c, indent + '  ')).join('\n');
  const text = data.textContent ? `\n${indent}  {/** ${data.textContent} */}` : '';
  const inner = (children ? '\n' + children : '') + text + (children || text ? '\n' + indent : '');

  return `${indent}<${data.tag}${classPart}${idPart}${extraAttrs}${ariaAttr}>${inner}</${data.tag}>`;
}

function buildChildMarkupHTML(data: ExtractedElement, indent: string): string {
  const ariaPart = ariaString(data.ariaAttributes, data.role);
  const classPart = data.classes.length > 0 ? ` class="${data.classes.join(' ')}"` : '';
  const idPart = data.id ? ` id="${data.id}"` : '';
  const extraAttrs = Object.entries(data.attributes)
    .map(([k, v]) => ` ${k}="${v}"`)
    .join('');
  const ariaAttr = ariaPart ? ` ${ariaPart}` : '';
  const textPart = data.textContent ? data.textContent : '';

  if (data.children.length === 0 && !data.textContent) {
    if (data.tag === 'img') return `${indent}<img src="${data.attributes['src'] || ''}" alt="${data.attributes['alt'] || ''}"${classPart} />`;
    if (['input', 'br', 'hr'].includes(data.tag)) return `${indent}<${data.tag}${classPart}${idPart}${extraAttrs}${ariaAttr} />`;
    return `${indent}<${data.tag}${classPart}${idPart}${extraAttrs}${ariaAttr}></${data.tag}>`;
  }

  const childrenHTML = data.children.map((c) => buildChildMarkupHTML(c, indent + '  ')).join('\n');
  const inner = textPart + (childrenHTML ? '\n' + childrenHTML + '\n' + indent : '');
...
function generateReact(data: ExtractedElement, config: Record<string, unknown>): string {
  ...
  const className = data.classes.length > 0 ? data.classes.join(' ') : componentName.toLowerCase();

  const ariaPart = includeA11y ? ariaString(data.ariaAttributes, data.role) : '';
  const classAttr = includeStyles ? ` className="${className}"` : '';
  const ariaAttr = ariaPart ? ` ${ariaPart}` : '';

  const childMarkup = data.children.map((c) => buildChildMarkupReact(c, '        ')).join('\n');
  const hasChildren = data.children.length > 0 || data.textContent;

  const innerContent = hasChildren
    ? (data.textContent ? `{/* ${data.textContent} */}` : '{children}') + (childMarkup ? '\n' + childMarkup : '')
    : '';
  ...
  const cssBlock = includeStyles && Object.keys(data.computedStyles).length > 0
    ? `\n/* Styles:\n${buildCSSBlock(className, data.computedStyles)}\n*/\n`
    : '';

  return `${cssBlock}import React from 'react';\n\n${propsInterface}export function ${componentName}({ children, className }: ${includeTypes ? componentName + 'Props' : 'any'}) {\n  return (\n    <${data.tag}${classAttr}${ariaAttr}>\n      ${innerContent}\n    </${data.tag}>\n  );\n}\n`;
}

// ---- Vue SFC ----

function generateVue(data: ExtractedElement, config: Record<string, unknown>): string {
  const componentName = generateComponentName(data);
  const includeTypes = config.includeTypes !== false;
  const includeStyles = config.includeStyles !== false;
  const includeA11y = config.includeAccessibility !== false;
  ...
  const childMarkup = data.children.map((c) => buildChildMarkupHTML(c, '    ')).join('\n');
  const hasChildren = data.children.length > 0 || data.textContent;
  const innerContent = hasChildren
    ? (data.textContent ? `\n      ${data.textContent}` : '\n      <slot />') + (childMarkup ? '\n' + childMarkup : '') + '\n    '
    : '';
  ...
  const styleBlock = includeStyles && Object.keys(data.computedStyles).length > 0
    ? `\n<style scoped>\n${buildCSSBlock(className, data.computedStyles)}\n</style>\n`
    : '';
...
function generateSvelte(data: ExtractedElement, config: Record<string, unknown>): string {
  const componentName = generateComponentName(data);
  const includeTypes = config.includeTypes !== false;
  const includeStyles = config.includeStyles !== false;
  const includeA11y = config.includeAccessibility !== false;
...
function generateHTML(data: ExtractedElement, config: Record<string, unknown>): string {
  const componentName = generateComponentName(data);
  const includeStyles = config.includeStyles !== false;
  const includeA11y = config.includeAccessibility !== false;
  const className = data.classes.length > 0 ? data.classes.join(' ') : componentName.toLowerCase();
  ...
  const hasChildren = data.children.length > 0 || data.textContent;
  const innerContent = hasChildren
    ? '\n  ' + (data.textContent || '<!-- content -->') + (childMarkup ? '\n' + childMarkup : '') + '\n'
    : '';
...
function generateCode(data: ExtractedElement, framework: Framework, config: Record<string, unknown>): string {
  switch (framework) {
    case 'react': return generateReact(data, config);
    case 'vue': return generateVue(data, config);
    case 'svelte': return generateSvelte(data, config);
    case 'html': return generateHTML(data, config);
  }
}

// ---------------------------------------------------------------------------
// Basic syntax highlighting (keyword/string/type coloring via spans)
// ---------------------------------------------------------------------------
...
function highlightCode(code: string, framework: Framework): string {
  let html = escapeHTML(code);

  // Keywords
  const keywords = ['import', 'export', 'from', 'const', 'let', 'function', 'return', 'interface', 'type', 'defineProps'];
  ...
  // Types (capitalized identifiers) for React
  if (framework === 'react') {
...
function calculateAccessibilityScore(data: ExtractedElement): { score: number; issues: string[] } {
  let score = 100;
  const issues: string[] = [];
  ...
  if (interactiveTags.includes(data.tag)) {
    ...
    if (data.tag === 'button' && !data.textContent && !data.ariaAttributes['aria-label']) {
      score -= 15;
...
function styledDiv(css: string): HTMLDivElement {
...
export const copyAsComponent: ToolDefinition = {
  ...
  configSchema: {
    ...
    includeTypes: { type: 'boolean', label: 'Include TypeScript Types', default: true },
    includeStyles: { type: 'boolean', label: 'Include Styles', default: true },
    includeAccessibility: { type: 'boolean', label: 'Include ARIA Attributes', default: true },
    makeResponsive: { type: 'boolean', label: 'Make Responsive', default: false },
  },
  ...
  run: (ctx, config) => {
    ...
    const headerTitle = styledSpan('font-weight:600;font-size:13px;color:#f1f5f9;', 'Copy as Component');

    const fw = (cfg.targetFramework as string) || 'react';
    const fwLabels: Record<string, string> = { react: 'React TSX', vue: 'Vue SFC', svelte: 'Svelte', html: 'HTML/CSS' };
    ...
    closeBtn.addEventListener('mouseenter', () => { closeBtn.style.color = '#f1f5f9'; closeBtn.style.background = '#334155'; });
    ...
    // -- Panel Body --
    const body = styledDiv('flex:1;overflow-y:auto;padding:12px 14px;min-height:0;');

    // Placeholder content
    const placeholder = styledDiv('color:#64748b;text-align:center;padding:32px 16px;line-height:1.6;');
    const placeholderIcon = styledDiv('font-size:28px;margin-bottom:8px;', '📦');
    const placeholderTitle = styledDiv('font-size:13px;color:#94a3b8;font-weight:500;', 'Hover over any element');
    const placeholderSub = styledDiv('margin-top:4px;', 'Click to select and generate component code');
    ...
    function updateTooltip(el: HTMLElement, rect: DOMRect): void {
      ...
      const tagLine = styledSpan('color:#f472b6;font-weight:600;', `<${tag}${id}${cls ? '.' + cls : ''}>`);
    ...
    function renderCode(code: string, a11yScore: number, a11yIssues: string[]): void {
      while (body.firstChild) body.removeChild(body.firstChild);

      // Accessibility score row
      const scoreRow = styledDiv('display:flex;align-items:center;gap:8px;margin-bottom:10px;');

      const scoreLabel = styledSpan('font-size:11px;color:#64748b;', 'Accessibility Score');

      const scoreValue = styledSpan('', String(a11yScore));
      const scoreColor = a11yScore >= 90 ? '#22c55e' : a11yScore >= 70 ? '#eab308' : '#ef4444';
      ...
      if (a11yIssues.length > 0) {
        const issuesWrap = styledDiv('margin-left:auto;display:flex;flex-direction:column;gap:2px;');
        for (const issue of a11yIssues.slice(0, 3)) {
          const issueEl = styledSpan('font-size:10px;color:#f871c0;background:#f871c015;padding:1px 6px;border-radius:3px;', issue);
      ...
      // Code block wrapper
      const codeWrap = styledDiv('background:#0c1222;border:1px solid #1e293b;border-radius:8px;overflow:hidden;');

      // Code block header
      const codeHeader = styledDiv('display:flex;align-items:center;justify-content:space-between;padding:6px 10px;background:#1e293b;border-bottom:1px solid #334155;');

      const codeLabel = styledSpan('font-size:11px;color:#64748b;', fwLabels[fw] || 'Code');

      const copyBtn = styledButton('Copy to Clipboard',
      ...
      codeContent.innerHTML = highlightCode(code, fw as Framework);
    ...
    function selectElement(el: HTMLElement): void {
      ...
      const sel = (typeof el.className === 'string' ? el.className.split(/\s+/)[0] : '') || el.tagName.toLowerCase();
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (panel.contains(target) || tooltip.contains(target) || highlight.contains(target)) return;
