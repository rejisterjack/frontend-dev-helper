...
import { addOverlayElement, removeOverlayElement, getOverlayContainer } from '@/content/overlay-manager';
...
function scanAccessibility(issues: XRayIssue[]): void {
  ...
  document.querySelectorAll('img').forEach((img) => {
    ...
    if (!img.alt || img.alt.trim() === '') {
      issues.push({
        ...
        description: 'This image has no alt attribute. Screen readers cannot convey its purpose.',
        element: img as HTMLImageElement,
        selector: getSelector(img),
        suggestedFix: 'Add a descriptive alt attribute, or role="presentation" if decorative.',
      });
    }
    if (img.alt && img.alt.length > 125) {
      issues.push({
        ...
        description: 'Alt text is ' + img.alt.length + ' characters (recommended max 125).',
        element: img as HTMLImageElement,
        selector: getSelector(img),
  ...
  document.querySelectorAll('input, select, textarea').forEach((input) => {
    ...
    if (!hasLabel) {
      issues.push({
        ...
        description: describeEl(el) + ' has no associated label, aria-label, or aria-labelledby.',
        element: el,
        selector: getSelector(el),
        suggestedFix: 'Add a <label for="..."> element, or an aria-label attribute.',
      });
    }
  });
  ...
  document.querySelectorAll('[role]').forEach((el) => {
    ...
    if (role && !validRoles.has(role)) {
      issues.push({
        ...
        selector: getSelector(el),
        suggestedFix: 'Replace with a valid ARIA role from the WAI-ARIA specification.',
      });
    }
  });
  ...
  document.querySelectorAll('[aria-hidden="true"]').forEach((el) => {
    ...
    if (focusable) {
      issues.push({
        ...
        description: 'A focusable element is inside a container with aria-hidden="true".',
        element: el as HTMLElement,
        selector: getSelector(el),
        suggestedFix: 'Remove aria-hidden or make descendant elements non-focusable.',
      });
    }
  });
  ...
  textEls.forEach((el) => {
    ...
    const ratio = contrastRatio(relativeLuminance(fgColor), relativeLuminance(bgColor));
    ...
    if (ratio < required) {
      issues.push({
        ...
        description: describeEl(htmlEl) + ' has a contrast ratio of ' + ratio.toFixed(1) + ':1 (required ' + required + ':1).',
        element: htmlEl,
        selector: getSelector(htmlEl),
        suggestedFix: 'Increase the contrast between text and background colour.',
      });
      checked++;
    }
  });
  ...
  if (h1Count === 0) {
    issues.push({
      ...
      description: 'The page has no <h1> element. Screen readers and SEO rely on a single h1.',
      selector: 'html',
  ...
  if (!lang || lang.trim() === '') {
    issues.push({
      ...
      description: 'The <html> element has no lang attribute. Screen readers need it to choose pronunciation rules.',
      selector: 'html',
...
function scanPerformance(issues: XRayIssue[]): void {
  ...
  document.querySelectorAll('img').forEach((img) => {
    ...
    if (naturalW > 1000 && displayW > 0 && naturalW > displayW * 2) {
      issues.push({
        ...
        description:
          'Image natural width is ' + naturalW + 'px but displayed at ~' +
          Math.round(displayW) + 'px. Serving a smaller file would save bandwidth.',
        element: htmlImg,
        selector: getSelector(htmlImg),
        suggestedFix: 'Use srcset / responsive images or resize the source file to ~' +
          Math.round(displayW * 2) + 'px wide.',
      });
    }

    // Missing width/height attributes (CLS risk)
    if (!htmlImg.getAttribute('width') && !htmlImg.getAttribute('height') && !htmlImg.style.aspectRatio) {
      issues.push({
        id: nextId(),
        severity: 'warning',
        category: 'performance',
        title: 'Image missing dimensions (CLS risk)',
        description: describeEl(htmlImg) + ' has no width/height attributes, which can cause layout shifts.',
        element: htmlImg,
        selector: getSelector(htmlImg),
        suggestedFix: 'Add width and height attributes to prevent Cumulative Layout Shift.',
      });
    }
  });
  ...
  document.querySelectorAll('script[src]').forEach((script) => {
    ...
    issues.push({
      ...
      description: 'Script in <head> without async/defer: ' + (el.src?.slice(0, 80) || 'inline'),
      element: el,
      selector: getSelector(el),
      suggestedFix: 'Add async or defer attribute, or move the script to the end of <body>.',
    });
  });
}
...
function scanSEO(issues: XRayIssue[]): void {
  ...
  if (!metaDesc || !metaDesc.getAttribute('content')?.trim()) {
    issues.push({
      ...
      selector: 'head',
      suggestedFix: 'Add <meta name="description" content="..."> with a concise page summary.',
    });
    ...
    issues.push({
      ...
      description: 'Meta description is ' + (metaDesc.getAttribute('content')?.length || 0) + ' chars (recommended max 160).',
  ...
  if (!titleEl || !titleEl.textContent?.trim()) {
    ...
    issues.push({
      ...
      description: 'Title is ' + titleEl.textContent!.length + ' chars (recommended max 60).',
      selector: 'title',
  ...
  if (!canonical) {
    issues.push({
      ...
      description: 'No <link rel="canonical"> found. Search engines may index duplicate URLs.',
      selector: 'head',
      suggestedFix: 'Add <link rel="canonical" href="..."> to specify the preferred URL.',
    });
  }
  ...
  if (h1s.length > 1) {
    h1s.forEach((h1) => {
      issues.push({
        ...
        description: 'Found ' + h1s.length + ' <h1> elements. Use only one per page for optimal SEO.',
        element: h1 as HTMLElement,
        selector: getSelector(h1),
        suggestedFix: 'Keep one <h1> as the primary heading and downgrade others to <h2>.',
      });
    });
  }
  ...
  if (!viewport) {
    issues.push({
      ...
      description: 'No <meta name="viewport"> found. Mobile friendliness affects rankings.',
      selector: 'head',
  ...
  if (!ogTitle) {
    issues.push({
      ...
      description: 'No Open Graph title found. Social sharing previews will fall back to <title>.',
      selector: 'head',
  ...
  document.querySelectorAll('[tabindex]').forEach((el) => {
    ...
    if (tabindex > 0) {
      issues.push({
        ...
        element: el as HTMLElement,
        selector: getSelector(el),
        suggestedFix: 'Use tabindex="0" and reorder DOM elements to achieve the desired tab order.',
      });
    }
  });
  ...
  document.querySelectorAll('div[onclick], span[onclick]').forEach((el) => {
    if (!el.getAttribute('tabindex') && !el.getAttribute('role')) {
      issues.push({
        ...
        description: describeEl(el) + ' has an onclick but no tabindex or role.',
        element: el as HTMLElement,
        selector: getSelector(el),
        suggestedFix: 'Add tabindex="0" and role="button", or use a native <button>.',
      });
    }
  });
  ...
  deprecatedTags.forEach((tag) => {
    document.querySelectorAll(tag).forEach((el) => {
      issues.push({
        ...
        description: 'The <' + tag + '> element is deprecated and should not be used.',
        element: el as HTMLElement,
        selector: getSelector(el),
  ...
  document.querySelectorAll('a[target="_blank"]').forEach((a) => {
    ...
    if (!rel.includes('noopener')) {
      issues.push({
        ...
        description: describeEl(a) + ' opens in a new tab but lacks rel="noopener", which is a security risk.',
        element: a as HTMLAnchorElement,
        selector: getSelector(a),
  ...
  document.querySelectorAll('[style]').forEach((el) => {
    ...
    if (style.includes('!important')) {
      issues.push({
        ...
        description: describeEl(el) + ' uses !important in an inline style, making overrides harder.',
        element: el as HTMLElement,
        selector: getSelector(el),
        suggestedFix: 'Move the style to a CSS class and increase specificity instead.',
      });
    }
  });
}
...
function createOutlineForIssue(issue: XRayIssue): HTMLElement | null {
  ...
  `;
  label.textContent = CATEGORY_LABELS[issue.category] + ': ' + issue.title.slice(0, 30);
...
function showTooltip(
  ...
  title.style.cssText = 'font-weight:600;font-size:13px;margin-bottom:4px;color:#f1f5f9;';
  ...
  if (issue.suggestedFix) {
    ...
    fixLabel.style.cssText = 'font-size:10px;color:#64748b;margin-bottom:2px;font-weight:600;text-transform:uppercase;';
    fixLabel.textContent = 'Suggested Fix';
    tooltip.appendChild(fixLabel);

    const fix = document.createElement('div');
    fix.style.cssText = 'color:#4ade80;font-size:11px;line-height:1.4;background:#0c1222;padding:4px 6px;border-radius:4px;';
...
function buildSummaryContent(
  ...
  const categories: XRayIssue['category'][] = ['accessibility', 'performance', 'seo', 'best-practice'];
  ...
  summary.appendChild(createBadge(criticalCount + ' Critical', SEVERITY_COLORS.critical));
  summary.appendChild(createBadge(warningCount + ' Warning', SEVERITY_COLORS.warning));
  ...
  for (const cat of categories) {
    ...
    `;
    groupHeader.textContent = cat.charAt(0).toUpperCase() + cat.slice(1).replace('-', ' ');
    ...
    chevron.style.cssText = 'margin-left:auto;font-size:10px;transition:transform 0.2s;';
    chevron.textContent = '▼';
    ...
    for (const issue of catIssues) {
      ...
      item.addEventListener('mouseleave', () => { item.style.background = '#1e293b'; });

      const titleRow = document.createElement('div');
      titleRow.style.cssText = 'font-size:11px;color:#f1f5f9;margin-bottom:2px;';
      titleRow.textContent = issue.title;
      item.appendChild(titleRow);

      if (issue.selector && issue.selector !== 'html' && issue.selector !== 'head' && issue.selector !== 'title') {
        const selectorRow = document.createElement('div');
        selectorRow.style.cssText = 'font-size:10px;color:#64748b;font-family:monospace;';
  ...
  if (issues.length === 0) {
    ...
    empty.style.cssText = 'padding:32px 16px;text-align:center;color:#4ade80;font-size:14px;';
...
export const xRayMode: ToolDefinition = {
  ...
  icon: 'scan-eye',
  configSchema: {
    checkAccessibility: { type: 'boolean', label: 'Accessibility', default: true },
    checkPerformance: { type: 'boolean', label: 'Performance', default: true },
    checkSEO: { type: 'boolean', label: 'SEO', default: true },
    checkBestPractices: { type: 'boolean', label: 'Best Practices', default: true },
  },
