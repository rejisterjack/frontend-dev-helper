...
import { addOverlayElement, removeOverlayElement, clearAllOverlays, createHighlightBox } from '@/content/overlay-manager';
...
function generateSelector(el: HTMLElement): string {
  ...
  while (current && current !== document.body) {
    ...
    if (current.className && typeof current.className === 'string') {
      const classes = current.className.trim().split(/\s+/).filter(c => !c.startsWith('fdh-')).join('.');
...
function createOverlayBadge(item: FocusableItem, isCurrent: boolean): HTMLDivElement {
...
export const focusDebugger: ToolDefinition = {
  ...
  icon: 'Focus',
  configSchema: {
    showFocusOrder: { type: 'boolean', label: 'Show Focus Order', default: true },
    showTabindex: { type: 'boolean', label: 'Show Tabindex', default: true },
    highlightFocused: { type: 'boolean', label: 'Highlight Focused Element', default: true },
  ...
  run: (ctx, config) => {
    ...
    panelHost.style.cssText = 'position:fixed;top:20px;right:20px;width:500px;max-height:80vh;z-index:2147483646;';
    ...
    function updateOverlays(): void {
      ...
      for (const item of focusableItems) {
        ...
        const overlay = createOverlayBadge(item, item.element === currentFocused);
    ...
    function renderList(): void {
      ...
      for (const item of focusableItems) {
        ...
        row.className = 'fdh-fd-item' + (item.element === currentFocused ? ' current' : '');
      ...
      statsEl.textContent = 'Focus: ' + (currentFocused ? findTabIndex() : '-') + ' / ' + focusableItems.length;
    }
    ...
    function handleFocusIn(e: FocusEvent): void {
      ...
      else if (e.relatedTarget === null && document.hasFocus()) trigger = 'mouse';

      focusHistory.unshift({ timestamp: Date.now(), element: target.tagName.toLowerCase(), selector: generateSelector(target), trigger });
    ...
    observer.observe(document.body, { childList: true, subtree: true, attributes: true, attributeFilter: ['tabindex', 'disabled', 'hidden', 'style'] });
    ...
    panel.addEventListener('click', (e) => {
      ...
      else if (action === 'toggle') { overlaysVisible = !overlaysVisible; updateOverlays(); }
    });
