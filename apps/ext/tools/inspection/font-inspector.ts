...
function detectFontSource(fontFamily: string): { source: string; label: string } {
  ...
  const typekitScripts = document.querySelectorAll('script[src*="typekit.net"], link[href*="use.typekit.net"]');
            ...
            if (src) {
              ...
              if (urlMatch) {
                try {
                  ...
                  return {
                    source: fontUrl.origin === location.origin ? 'self-hosted' : 'cdn',
                    label: fontUrl.origin === location.origin ? 'Self-hosted' : 'CDN',
                  };
                } catch {
  ...
  const systemFonts = [
    ...
    'georgia', 'system-ui', '-apple-system', 'blinkmacsystemfont',
    'segoe ui', 'roboto', 'sans-serif', 'serif', 'monospace',
  ];
...
function isValidTextElement(element: HTMLElement): boolean {
  if (element.id?.startsWith('fdh-') || element.closest('[id^="fdh-"]')) return false;

  const tagName = element.tagName.toLowerCase();
  if (['script', 'style', 'meta', 'link', 'head', 'html', 'body'].includes(tagName)) return false;
...
export const fontInspector: ToolDefinition = {
  ...
  icon: 'Type',
  configSchema: {
    showFontFamily: { type: 'boolean', label: 'Show Font Family', default: true },
    showFontSize: { type: 'boolean', label: 'Show Font Size', default: true },
    showLineHeight: { type: 'boolean', label: 'Show Line Height', default: true },
    showFontWeight: { type: 'boolean', label: 'Show Font Weight', default: true },
    showLetterSpacing: { type: 'boolean', label: 'Show Letter Spacing', default: false },
    highlightOnHover: { type: 'boolean', label: 'Highlight on Hover', default: true },
  },
  ...
  run(ctx, config) {
    ...
    const originalOutlines = new WeakMap<HTMLElement, { outline: string; outlineOffset: string }>();
    ...
    function showTooltip(element: HTMLElement, mouseX: number, mouseY: number): void {
      if (!tooltip) return;
      ...
      const lineHeight = cs.lineHeight === 'normal' ? 'normal' : `${parseFloat(cs.lineHeight).toFixed(2)}`;
      ...
      header.style.cssText = 'display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;padding-bottom:8px;border-bottom:1px solid #334155';

      const familySpan = document.createElement('span');
      familySpan.style.cssText = 'font-weight:600;font-size:14px;color:#f8fafc;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:200px';
      familySpan.textContent = primaryFamily;

      const badge = document.createElement('span');
      badge.style.cssText = 'font-size:10px;padding:2px 6px;border-radius:4px;background:#3b82f6;color:white;text-transform:uppercase;font-weight:500';
      ...
      const addRow = (label: string, value: string) => {
        ...
        val.style.cssText = 'color:#e2e8f0;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:12px';
      ...
      if (showLetterSpacing) addRow('Letter Spacing', letterSpacing === 'normal' ? 'normal' : letterSpacing);
      addRow('Color', color);
      ...
      requestAnimationFrame(() => {
        ...
        if (left + ttRect.width > window.innerWidth) left = mouseX - ttRect.width - 15;
        if (top + ttRect.height > window.innerHeight) top = mouseY - ttRect.height - 15;
    ...
    const handleMouseOut = (e: MouseEvent) => {
      const target = e.target as HTMLElement;
      if (target === highlightedElement || target === highlightedElement?.parentElement) {
        removeHighlightEl();
        hideTooltip();
      }
    };
