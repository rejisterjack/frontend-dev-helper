...
function createFileRow(
  ...
  if (onClick) {
    row.addEventListener("mouseenter", () => {
      row.style.background = COLORS.bgLighter;
    });
    row.addEventListener("mouseleave", () => {
      row.style.background = COLORS.bgLight;
    });
...
export const sourceMapViewer: ToolDefinition = {
  ...
  run(ctx, config) {
    ...
    const autoScan = (config?.autoScan ?? true) as boolean;
    ...
    function showFileList(): void {
      ...
      if (enrichedInfos.length === 0) {
        contentArea.appendChild(
          createEmptyState("No source maps found on this page."),
        );
        return;
      }
      ...
      for (const info of enrichedInfos) {
        ...
        if (info.verified) {
          topRow.appendChild(
            createBadge(`${info.sources.length} sources`, COLORS.accent),
          );
        } else {
    ...
    function showFileDetail(info: EnrichedInfo): void {
      ...
      for (const source of info.sources) {
        ...
        srcRow.style.cssText = `
          ...
          transition: background 0.15s;
        `;
        srcRow.addEventListener("mouseenter", () => {
          srcRow.style.background = COLORS.bgLighter;
        });
        srcRow.addEventListener("mouseleave", () => {
          srcRow.style.background = COLORS.bgLight;
        });
    ...
    async function showSourceContent(
      info: EnrichedInfo,
      sourceFile: string,
    ): Promise<void> {
      clearChildren(contentArea);
      ...
      try {
        ...
        if (content !== null) {
          ...
          contentArea.appendChild(
            createEmptyState("Source content not embedded in the source map."),
          );
        }
      } catch (err) {
        loadingDiv.remove();
        ...
        try {
          ...
          if (enriched) {
            ...
            results.push({
              ...raw,
              verified: false,
              fetchError: "Source map not found or invalid",
            });
          }
        } catch (err) {
          results.push({
    ...
    // --- Start ---
    // When autoScan is enabled (default) the viewer fetches source maps on
    // open. When disabled, the user must click "Rescan" to populate the list —
    // useful on heavy pages where automatic fetching would slow the panel.
    if (autoScan) {
      scan();
    } else {
      showFileList();
