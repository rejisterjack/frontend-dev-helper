...
export const elementInspector: ToolDefinition = {
  ...
  run(ctx, config) {
    ...
    const showEventListeners = (config?.showEventListeners ?? false) as boolean;
    const showAccessibility = (config?.showAccessibility ?? true) as boolean;
    const maxStyles = (config?.maxStyles ?? 20) as number;
    // When showEventListeners is enabled, the inspector surfaces a small
    // "events" section enumerating inline handlers (`onclick`, `onload`, etc.)
    // discovered on the inspected element. The collector is defined here so
    // the tooltip builder can call it via the closure.
    const collectInlineListeners = (el: HTMLElement): string[] => {
      if (!showEventListeners) return [];
    ...
    let isPinned = false;
    let currentSource: { file: string; line: number; column: number } | null =
      null;
    ...
    function buildTooltipContent(el: HTMLElement): void {
      ...
      header.style.cssText =
        "font-weight:600;margin-bottom:4px;font-size:13px;display:flex;justify-content:space-between;align-items:center";
      const headerLeft = document.createElement("span");
      appendStyledSpan(headerLeft, `<${tag}>`, "#89b4fa");
      if (el.id) appendStyledSpan(headerLeft, `#${el.id}`, "#f38ba8");
      if (classes.length > 0)
        appendStyledSpan(headerLeft, `.${classes.join(".")}`, "#a6e3a1");
      ...
      if (isPinned && currentSource) {
        ...
        vscodeBtn.style.cssText =
          "pointer-events:auto;background:#6366f1;color:#fff;border:none;border-radius:4px;padding:2px 8px;font-size:10px;cursor:pointer;font-family:inherit";
      ...
      if (showBoxModel) {
        ...
        boxLabel.style.cssText =
          "color:#94a3b8;font-size:10px;margin-bottom:2px";
      ...
      if (showAccessibility) {
        ...
        if (role || ariaLabel || tabIndex !== null) {
          ...
          a11ySection.style.cssText =
            "margin-top:4px;padding-top:4px;border-top:1px solid rgba(255,255,255,0.1)";
          const a11yLabel = document.createElement("div");
          a11yLabel.style.cssText =
            "color:#94a3b8;font-size:10px;margin-bottom:2px";
          ...
          if (ariaLabel)
            appendStyledSpan(
              a11ySection,
              `aria-label="${ariaLabel.slice(0, 30)}" `,
              "#fbbf24",
            );
          if (tabIndex !== null)
            appendStyledSpan(a11ySection, `tabindex=${tabIndex}`, "#fbbf24");
          tooltip.appendChild(a11ySection);
        }
      }

      // Event listeners (inline `on*` handlers). Gated by the showEventListeners
      // config so the section only renders when the user opts in.
      const listeners = collectInlineListeners(el);
      if (listeners.length > 0) {
        const evSection = document.createElement("div");
        evSection.style.cssText =
          "margin-top:4px;padding-top:4px;border-top:1px solid rgba(255,255,255,0.1)";
        const evLabel = document.createElement("div");
        evLabel.style.cssText =
          "color:#94a3b8;font-size:10px;margin-bottom:2px";
        evLabel.textContent = "Event Listeners";
        evSection.appendChild(evLabel);
        appendStyledSpan(evSection, listeners.join(", "), "#a78bfa");
        tooltip.appendChild(evSection);
      }
      ...
      hint.textContent = isPinned
        ? "Click again to unpin | ESC to unpin"
        : "Click to pin | ESC to exit";
