...
function classifyHookName(
  ...
  if (
    ...
    return stateNode.queue?.name
      ? `useState(${stateNode.queue.name})`
      : "useState";
  }
  if (tag === HOOK_TAG_EFFECT || tag === HOOK_TAG_LAYOUT_EFFECT) {
    return tag === HOOK_TAG_LAYOUT_EFFECT ? "useLayoutEffect" : "useEffect";
  }
  if (
    tag === HOOK_TAG_REF &&
    ...
    if ("current" in val) return "useRef";
  }
  if (tag === 8) {
    return typeof stateNode.memoizedState === "function"
      ? "useCallback"
      : "useMemo";
  }
  if (tag === 9 || tag === 10) return "useContext";
  return tag !== undefined ? `hook(tag=${tag})` : "hook";
}
...
function detectFramework(): FrameworkType {
  ...
  if (
    ...
    return "react";
  if (w.__VUE__ || document.querySelector("[data-v-app], [data-vue-root]"))
    return "vue";
  if (w.ng || document.querySelector("[ng-app], [ng-version]"))
    return "angular";
...
function getVersion(framework: FrameworkType): string | undefined {
  ...
  if (framework === "vue")
    return (w.Vue as { version?: string } | undefined)?.version;
...
function getReactComponentInfo(
  ...
  while (state) {
    hooks.push(classifyHookName(state));
...
function getVueComponentInfo(element: HTMLElement): FrameworkComponent | null {
  ...
  while (target) {
    ...
    const parentComp = el.__vueParentComponent as
      ...
      | undefined;
    if (parentComp) {
      const type = parentComp.type as Record<string, unknown> | undefined;
      const name =
        (type?.name as string | undefined) ||
        (type?.__name as string | undefined) ||
        (parentComp.name as string | undefined) ||
        "Anonymous";
      const props = (parentComp.props as Record<string, unknown>) || {};
      const setupState = parentComp.setupState as
        | Record<string, unknown>
        | undefined;
      return {
        name,
    ...
    // Vue 2 fallback
    const vue2 = el.__vue__ as Record<string, unknown> | undefined;
    if (vue2) {
      return {
        name:
          ((vue2.$options as Record<string, unknown>)?.name as string) ||
          "Anonymous",
        framework: "vue",
        props: (vue2.$props as Record<string, unknown>) || {},
        state: (vue2.$data as Record<string, unknown>) || {},
        domElement: element,
      };
    }
...
function getAngularComponentInfo(
  _element: HTMLElement,
): FrameworkComponent | null {
  // Angular does not expose component instances on DOM nodes; rely on DevTools hooks instead.
  return null;
}
...
export const frameworkDevtools: ToolDefinition = {
  ...
  run: (ctx, config) => {
    ...
    panelHost.style.cssText =
      "position:fixed;top:20px;right:20px;width:400px;max-height:75vh;z-index:2147483646;";
    ...
    function renderFrameworkInfo(): void {
      ...
      devtoolsDiv.style.background = devtoolsAvailable
      ...
      statusText.textContent = devtoolsAvailable
        ? "DevTools hooks detected"
        : "No DevTools hooks found";
      ...
      if (framework === "none") {
        ...
        hint.textContent =
          "Move your mouse over elements to see component info.";
    ...
    function handleMouseMove(e: MouseEvent): void {
      ...
      if (currentHighlight) {
        removeOverlayElement(currentHighlight);
        currentHighlight = null;
      }
      if (currentCompCard) {
        currentCompCard.remove();
        currentCompCard = null;
      }
      ...
      if (comp.props && Object.keys(comp.props).length > 0) {
        const filtered = Object.entries(comp.props).filter(
          ([k]) =>
            !k.startsWith("__") &&
            k !== "children" &&
            k !== "key" &&
      ...
      if (comp.hooks && comp.hooks.length > 0) {
        ...
        hooksEl.style.cssText =
          "font-size:11px;color:#89b4fa;font-family:monospace;margin-top:4px;";
