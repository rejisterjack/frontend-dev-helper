...
export const domOutliner: ToolDefinition = {
  ...
  run(ctx, config) {
    ...
    function positionOutline(entry: OutlineEntry): void {
    ...
    function clearOutlines(): void {
      for (const entry of outlineEntries) removeOverlayElement(entry.outline);
    ...
    function walkDOM(): void {
      ...
      for (const el of allElements) {
        ...
        const color =
          customColor || ELEMENT_COLORS[tag] || getDepthColor(depth);
        ...
        outlineEntries.push({ element: htmlEl, outline });
    ...
    const detachViewportTracker = attachViewportTracker(() => {
      for (const entry of outlineEntries) positionOutline(entry);
    });

    const hoverHandler = (e: MouseEvent) => {
      const target = e.target as Element;
      if (!target || (target as HTMLElement).classList?.contains(OVERLAY_CLASS))
        return;
      ...
      if (left + ttRect.width > window.innerWidth)
        left = window.innerWidth - ttRect.width - offset;
