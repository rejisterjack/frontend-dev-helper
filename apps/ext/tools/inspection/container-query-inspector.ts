...
function detectContainers(): ContainerInfo[] {
  ...
  while ((node = walker.nextNode())) {
    ...
    if (containerType && containerType !== 'normal') {
      ...
      containers.push({
        element,
        name: containerName && containerName !== 'none' ? containerName : null,
        type: containerType,
        width: rect.width,
        height: rect.height,
        queries,
...
function getContainerQueriesForElement(element: HTMLElement): { condition: string }[] {
  const queries: { condition: string }[] = [];
      ...
      for (const rule of sheet.cssRules) {
        if (rule instanceof CSSContainerRule) {
          ...
          while (parent) {
            ...
            if (ct && ct !== 'normal') {
              if (!containerName || cs.containerName === containerName) {
                queries.push({ condition: rule.containerQuery });
                break;
              }
            }
...
export const containerQueryInspector: ToolDefinition = {
  ...
  icon: 'Containers',
  configSchema: {
    highlightContainers: { type: 'boolean', label: 'Highlight Containers', default: true },
    showBreakpoints: { type: 'boolean', label: 'Show Breakpoints', default: true },
    showMatchedRules: { type: 'boolean', label: 'Show Matched Rules', default: true },
    liveResize: { type: 'boolean', label: 'Live Resize Preview', default: true },
  },
  run: (ctx, config) => {
    const highlightContainers = (config?.highlightContainers as boolean) ?? true;
    ...
    panelHost.style.cssText = 'position:fixed;top:20px;right:20px;width:420px;max-height:75vh;z-index:2147483646;';
    ...
    function updateOverlays(containers: ContainerInfo[]): void {
      ...
      for (const info of containers) {
        ...
        label.style.cssText = 'position:absolute;top:-22px;left:0;background:#a855f7;color:white;padding:3px 8px;font-size:10px;font-family:monospace;border-radius:4px 4px 0 0;white-space:nowrap;font-weight:600;';
        ...
        if (showBreakpoints) {
          ...
          dims.style.cssText = 'position:absolute;bottom:4px;right:4px;background:rgba(168,85,247,0.9);color:white;padding:2px 6px;font-size:10px;font-family:monospace;border-radius:3px;';
          dims.textContent = Math.round(info.width) + 'x' + Math.round(info.height);
    ...
    function render(containers: ContainerInfo[]): void {
      ...
      const inlineCount = containers.filter(c => c.type === 'inline-size').length;
      ...
      for (const info of containers) {
        ...
        nameEl.textContent = info.name ? '@container ' + info.name : '@container (anonymous)';
        ...
        dimsEl.textContent = Math.round(info.width) + ' x ' + Math.round(info.height) + 'px';
        ...
        if (showMatchedRules && info.queries.length > 0) {
          for (const q of info.queries.slice(0, 3)) {
            ...
            qEl.textContent = '@container ' + q.condition;
            card.appendChild(qEl);
          }
          if (info.queries.length > 3) {
            const more = document.createElement('div');
            more.style.cssText = 'font-size:10px;color:#6c7086;text-align:center;padding-top:4px;';
            more.textContent = '+' + (info.queries.length - 3) + ' more queries';
