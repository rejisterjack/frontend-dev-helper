...
const AUTOFILL_FIELDS = [
  ...
  'postal-code', 'country', 'organization', 'username', 'new-password',
  'current-password', 'one-time-code', 'cc-name', 'cc-number', 'cc-exp',
];
...
function generateSelector(el: HTMLElement): string {
  ...
  const classes = Array.from(el.classList).filter(c => !c.startsWith('fdh-')).join('.');
  if (classes) return tag + '.' + classes;
  if ((el as HTMLInputElement).name) return tag + '[name="' + (el as HTMLInputElement).name + '"]';
  return tag;
}
...
function getLabelInfo(field: HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement): { hasLabel: boolean; text?: string } {
  if (field.id) {
    ...
    if (explicit) return { hasLabel: true, text: explicit.textContent || undefined };
  }
  const parentLabel = field.closest('label');
  if (parentLabel) return { hasLabel: true, text: parentLabel.textContent || undefined };
  const ariaLabel = field.getAttribute('aria-label');
  if (ariaLabel) return { hasLabel: true, text: ariaLabel };
  const ariaLabelledBy = field.getAttribute('aria-labelledby');
  if (ariaLabelledBy) {
    const labelEl = document.getElementById(ariaLabelledBy);
    if (labelEl) return { hasLabel: true, text: labelEl.textContent || undefined };
  }
...
function detectAutofill(field: HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement): string | undefined {
  ...
  if (autocomplete && AUTOFILL_FIELDS.includes(autocomplete)) return autocomplete;
...
function analyzeFormFields(form: HTMLFormElement): FormField[] {
  const fields = Array.from(form.querySelectorAll<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>('input, select, textarea'));
...
function checkAccessibilityIssues(form: HTMLFormElement, fields: FormField[]): string[] {
  ...
  if (unlabeled.length > 0) issues.push(unlabeled.length + ' fields without proper labels');
...
function analyzeForms(): FormInfo[] {
  ...
  const orphaned = Array.from(document.querySelectorAll('input:not(form input), select:not(form select), textarea:not(form textarea)'));
...
function createFieldOverlay(field: FormField): HTMLDivElement | null {
  const rect = (field.element as HTMLElement).getBoundingClientRect();
  if (rect.bottom < 0 || rect.top > window.innerHeight || rect.width === 0) return null;

  const color = field.hasError ? '#dc2626' : !field.hasLabel ? '#f59e0b' : '#3b82f6';
  const label = field.hasError ? (field.validationMessage || 'Invalid') : !field.hasLabel ? 'No label' : '';
...
export const formDebugger: ToolDefinition = {
  ...
  icon: 'FileInput',
  configSchema: {
    showValidation: { type: 'boolean', label: 'Show Validation', default: true },
    showConstraints: { type: 'boolean', label: 'Show Constraints', default: true },
    showLabels: { type: 'boolean', label: 'Show Labels', default: true },
    showDefaultValues: { type: 'boolean', label: 'Show Default Values', default: false },
    highlightRequired: { type: 'boolean', label: 'Highlight Required', default: true },
  },
  run: (ctx, config) => {
    ...
    panelHost.style.cssText = 'position:fixed;top:20px;right:20px;width:550px;max-height:80vh;z-index:2147483646;';
    ...
    function renderList(): void {
      ...
      const validFields = forms.reduce((s, f) => s + f.fields.filter(ff => ff.isValid).length, 0);
      ...
      for (const form of forms) {
        ...
        methodSpan.style.cssText = 'background:#334155;padding:2px 6px;border-radius:4px;font-size:10px;text-transform:uppercase;';
        ...
        for (const field of form.fields) {
          ...
          fieldEl.className = 'fdh-fg-field' + (field.hasError ? ' error' : '') + (!field.hasLabel ? ' unlabeled' : '');
    ...
    observer.observe(document.body, { childList: true, subtree: true, attributes: true, attributeFilter: ['name', 'type', 'required', 'disabled'] });

    panel.addEventListener('click', (e) => {
      const target = e.target as HTMLElement;
      const action = target.dataset.action;
      if (action === 'close') cleanup();
      else if (action === 'refresh') { refresh(); statusEl.textContent = 'Refreshed'; setTimeout(() => { statusEl.textContent = 'Ready'; }, 2000); }
