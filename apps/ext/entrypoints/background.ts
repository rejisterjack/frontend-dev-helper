...
import { sendChatMessage } from '@/lib/llm-service';
...
function registerBridgeRequestHandlers(): void {
  ...
  const forwardToContentScript = async (method: string, params: Record<string, unknown>): Promise<unknown> => {
  ...
  bridge.onRequest('inspectElement', (_, params) => forwardToContentScript('inspectElement', params));
  bridge.onRequest('getPageContext', async () => {
    const tabId = await getActiveTabId();
    if (!tabId) throw new Error('No active tab');
    return collectPageContext(tabId);
  });
  bridge.onRequest('getDiagnostics', (_, params) => forwardToContentScript('getDiagnostics', params));
  bridge.onRequest('getCssRules', (_, params) => forwardToContentScript('getCssRules', params));
  bridge.onRequest('scanAccessibility', (_, params) => forwardToContentScript('scanAccessibility', params));
  bridge.onRequest('getSuggestions', (_, params) => forwardToContentScript('getSuggestions', params));
  bridge.onRequest('jumpToSource', (_, params) => forwardToContentScript('jumpToSource', params));
  bridge.onRequest('getComponentTree', (_, params) => forwardToContentScript('getComponentTree', params));
}
...
const pendingDebounce = new Map<string, {
...
async function getActiveToolsCount(tabId?: number): Promise<number> {
  ...
  const state = result['fdh-tools-storage'];
  if (!state?.state?.activeTools) return 0;
  return Object.values(state.state.activeTools).filter((t: unknown) => (t as { active: boolean }).active).length;
}
...
async function updateBadge(tabId?: number): Promise<void> {
  try {
    ...
    await browser.action.setBadgeText({ text: count > 0 ? String(count) : '', tabId });
    await browser.action.setBadgeBackgroundColor({ color: count > 0 ? '#10B981' : '#6B7280' });
  } catch {
...
async function actuallyToggleTool(
  ...
  try {
    if (newEnabled) {
      await browser.tabs.sendMessage(tabId, { type: 'BG_ACTIVATE_TOOL', toolId, config });
    } else {
      await browser.tabs.sendMessage(tabId, { type: 'BG_DEACTIVATE_TOOL', toolId });
    }
  } catch {
    // Content script may not be injected
  }

  activeTools[toolId] = { toolId, active: newEnabled, config: activeTools[toolId]?.config ?? config ?? {}, activatedAt: Date.now() };
  await browser.storage.local.set({ 'fdh-tools-storage': { state: { ...state?.state, activeTools } } });

  try {
    await browser.runtime.sendMessage({ type: 'BG_TOOL_STATE_CHANGED', toolId, enabled: newEnabled });
  } catch {
    // Popup may be closed
  }
...
async function getAIConfig(): Promise<{ apiKey: string; model: string; baseUrl: string; provider: string } | null> {
  const result = await browser.storage.local.get('fdh-settings-storage');
  const settings = result['fdh-settings-storage']?.state;
  if (!settings?.ai?.enabled) return null;
  const provider = settings.ai.provider || 'openrouter';
  if (provider !== 'ollama' && !settings.ai?.apiKey) return null;
  return {
...
async function collectPageContext(tabId: number): Promise<PageContextData | null> {
  try {
    const response = await browser.tabs.sendMessage(tabId, { type: 'BG_COLLECT_PAGE_CONTEXT' });
    ...
    return {
      ...
      domStats: { totalElements: 0, images: 0, links: 0, headings: 0, scripts: 0, stylesheets: 0, forms: 0, inputs: 0, imagesWithoutAlt: 0, inputsWithoutLabel: 0 },
      headingHierarchy: [],
      performance: { domContentLoaded: 0, loadComplete: 0, domInteractive: 0, transferSize: 0 },
...
function buildContextMenus() {
  browser.contextMenus.removeAll(() => {
    ...
    browser.contextMenus.create({ id: 'fdh-sep1', type: 'separator', contexts: ['page'] });

    // Inspection tools submenu
    browser.contextMenus.create({ id: 'fdh-inspection', title: 'FDH: Inspection', contexts: ['page'] });
    const inspectionTools = [
      ...
      { id: 'component-tree', title: 'Component Tree' },
      { id: 'tech-detector', title: 'Tech Detector' },
    ];
    for (const tool of inspectionTools) {
      browser.contextMenus.create({ id: `fdh-toggle-${tool.id}`, title: tool.title, parentId: 'fdh-inspection', contexts: ['page'] });
    }

    // CSS tools submenu
    browser.contextMenus.create({ id: 'fdh-css-tools', title: 'FDH: CSS Tools', contexts: ['page'] });
    const cssTools = [
    ...
    for (const tool of cssTools) {
      browser.contextMenus.create({ id: `fdh-toggle-${tool.id}`, title: tool.title, parentId: 'fdh-css-tools', contexts: ['page'] });
    }

    // Performance tools submenu
    browser.contextMenus.create({ id: 'fdh-performance', title: 'FDH: Performance', contexts: ['page'] });
    const perfTools = [
      { id: 'network-analyzer', title: 'Network Analyzer' },
      { id: 'performance-budget', title: 'Performance Budget' },
      { id: 'flame-graph', title: 'Flame Graph' },
    ];
    for (const tool of perfTools) {
      browser.contextMenus.create({ id: `fdh-toggle-${tool.id}`, title: tool.title, parentId: 'fdh-performance', contexts: ['page'] });
    }

    // Accessibility tools submenu
    browser.contextMenus.create({ id: 'fdh-accessibility', title: 'FDH: Accessibility', contexts: ['page'] });
    const a11yTools = [
      { id: 'accessibility-audit', title: 'Accessibility Audit' },
      { id: 'focus-debugger', title: 'Focus Debugger' },
    ];
    for (const tool of a11yTools) {
      browser.contextMenus.create({ id: `fdh-toggle-${tool.id}`, title: tool.title, parentId: 'fdh-accessibility', contexts: ['page'] });
    }

    browser.contextMenus.create({ id: 'fdh-sep2', type: 'separator', contexts: ['page'] });
...
export default defineBackground(() => {
  ...
  browser.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    switch (msg.type) {
      ...
      case 'POPUP_DEACTIVATE_ALL': {
        ...
        if (tabId) {
          browser.tabs.sendMessage(tabId, { type: 'BG_DEACTIVATE_ALL' }).catch(() => {});
        }
      ...
      case 'POPUP_GET_SETTINGS': {
        browser.storage.local.get('fdh-settings-storage').then((result) => {
          const state = result['fdh-settings-storage'];
      ...
      case 'POPUP_UPDATE_SETTINGS': {
        browser.storage.local.set({ 'fdh-settings-storage': { state: msg.settings } });
        sendResponse({ success: true });
        return false;
      }
      ...
      case 'POPUP_SEND_AI_PROMPT': {
        (async () => {
          ...
          if (!aiConfig) {
            sendResponse({ error: 'AI not configured. Open Settings to add your API key.' });
            return;
          }
      ...
      case 'POPUP_GET_AI_HISTORY': {
        browser.storage.local.get('fdh-chat-history').then((result) => {
          sendResponse(result['fdh-chat-history'] ?? []);
        });
        return true;
      }
      ...
      case 'DEVTOOLS_ACTIVATE_TOOL': {
        ...
        if (tabId) {
          ...
          handleToggleTool({ toolId: msg.toolId, config: msg.config }, { tab: { id: tabId } } as browser.Runtime.MessageSender)
            .then((result) => sendResponse(result))
            .catch((err) => sendResponse({ error: String(err) }));
        }
        return true;
      ...
      case 'DEVTOOLS_DEACTIVATE_TOOL': {
        const tabId = msg.tabId;
        if (tabId) {
          browser.tabs.sendMessage(tabId, { type: 'BG_DEACTIVATE_TOOL', toolId: msg.toolId }).catch(() => {});
          handleToggleTool({ toolId: msg.toolId! }, { tab: { id: tabId } } as browser.Runtime.MessageSender)
            .then((result) => sendResponse(result))
            .catch((err) => sendResponse({ error: String(err) }));
        }
        return true;
      ...
      case 'VSCODE_JUMP_TO_SOURCE': {
        const bridge = getBridge();
        const payload = (message as { type: string; payload: { file: string; line: number; column: number } }).payload;
      ...
      case 'VSCODE_INIT_BRIDGE': {
        const payload = (message as { type: string; payload: { port: number } }).payload;
        initBridge(payload.port);
      ...
      case 'VSCODE_MESSAGE': {
        const vscodeMsg = (message as { type: string; message: unknown }).message;
        const bridge = getBridge();
        if (bridge.connected && vscodeMsg) {
          bridge.send(vscodeMsg as import('@/lib/vscode-protocol').VSCodeMessage);
        }
        sendResponse({ success: bridge.connected });
        return false;
      }
  ...
  browser.commands.onCommand.addListener((command, tab) => {
    ...
    if (toolId && tab?.id) {
      handleToggleTool({ toolId }, { tab } as browser.Runtime.MessageSender);
      return;
    }

    if (command === 'disable-all-tools' && tab?.id) {
      browser.tabs.sendMessage(tab.id, { type: 'BG_DEACTIVATE_ALL' }).catch(() => {});
      browser.storage.local.get('fdh-tools-storage').then((result) => {
        const state = result['fdh-tools-storage'];
        if (state?.state?.activeTools) {
          for (const key of Object.keys(state.state.activeTools)) {
  ...
  browser.contextMenus.onClicked.addListener((info, tab) => {
    ...
    if (menuItemId === 'fdh-inspect-element') {
      handleToggleTool({ toolId: 'element-inspector' }, { tab } as browser.Runtime.MessageSender);
    } else if (menuItemId === 'fdh-copy-selector') {
      browser.tabs.sendMessage(tabId, { type: 'BG_COPY_CSS_SELECTOR' }).catch(() => {});
