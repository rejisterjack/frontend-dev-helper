/**
 * Timeline — D3 commit timeline with bars colored by severity.
 *
 * Ported from react-perf-profiler/apps/ext/src/panel/components/visualizations/Timeline.tsx.
 */
...
const Timeline: React.FC<TimelineProps> = ({
  ...
  useEffect(() => {
    ...
    const barWidth = Math.max(Math.min(innerWidth / commits.length - 2, 24), 2);
    ...
    const xAxis = d3
      ...
      .tickFormat((d: Date | d3.NumberValue) =>
        timeFormatter(new Date(d.valueOf())),
      );
    ...
    g.selectAll("rect.bar")
      ...
      .attr("fill-opacity", (d) => (selectedCommitId === d.id ? 1 : 0.7))
      .attr("stroke", (d) => (selectedCommitId === d.id ? "#ffffff" : "none"))
      ...
      .on("mouseenter", function (event, d) {
        ...
        if (tooltip) {
          ...
          const nodeCount = d.fibers?.length ?? collectCommitFibers(d).length;
          ...
          const lines = [
            ...
            {
              text: `Duration: ${d.duration.toFixed(2)}ms`,
              cls: "text-xs text-muted-foreground",
            },
            {
              text: `Components: ${nodeCount}`,
              cls: "text-xs text-muted-foreground",
            },
            { text: `Time: ${timeStr}`, cls: "text-xs text-muted-foreground" },
            {
              text: `Priority: ${d.priorityLevel}`,
              cls: "text-xs text-muted-foreground",
            },
          ];
