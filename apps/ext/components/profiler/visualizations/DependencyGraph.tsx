 ...
 *
 * Ported from react-perf-profiler/apps/ext/src/panel/components/visualizations/DependencyGraph.tsx.
 * Adapted to read from FDH's useProfilerStore.
 */
...
function extractDependencyGraph(
  commits: Array<{
    renderCauses?: Array<{
      ...
      causes: Array<{ type: string; details: string }>;
    }>;
  }>,
...
export function DependencyGraph() {
  ...
  const { nodes, edges: rawEdges } = useMemo(
    () => extractDependencyGraph(commits),
    [commits],
  );
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Which components cause others to re-render
          </CardDescription>
        </CardHeader>
        <CardContent className="flex flex-col items-center justify-center py-8 text-muted-foreground gap-2">
          ...
          <p className="text-[10px]">
            Profile your app to build the dependency graph. Enable render cause
          ...
          <CardHeader className="pb-2">
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription>
                  {positionedNodes.length} components · {positionedEdges.length}{" "}
                  dependencies
                </CardDescription>
              </div>
              <div className="flex items-center gap-2 flex-wrap">
                {Object.entries(GROUP_COLORS).map(
                  ([group, { fill, label }]) => {
                    const count = positionedNodes.filter(
                      (n) => n.group === group,
                    ).length;
          ...
          <CardContent>
            ...
            >
              ...
              {positionedNodes.map((node) => {
                const colors =
                  GROUP_COLORS[node.group] ?? GROUP_COLORS["child"];
