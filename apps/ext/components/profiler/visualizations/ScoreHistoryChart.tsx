/**
 * ScoreHistoryChart — line chart of perf scores across sessions.
 *
 * Ported from react-perf-profiler/apps/ext/src/panel/components/visualizations/ScoreHistoryChart.tsx.
 */
...
} from "@/stores/use-profiler-store";
import {
  Card,
  CardContent,
  CardHeader,
...
function getTrend(entries: ScoreHistoryEntry[]): "up" | "down" | "stable" {
  ...
  const avgFirst =
    recent.slice(0, half).reduce((s, e) => s + e.score, 0) / half;
  const avgLast =
    recent.slice(half).reduce((s, e) => s + e.score, 0) /
    Math.max(1, recent.length - half);
...
export function ScoreHistoryChart() {
  ...
  const { pathD, points, trend } = useMemo(() => {
    ...
    const maxScore = Math.min(
      100,
      Math.max(...entries.map((e) => e.score)) + 5,
    );
    const scoreRange = maxScore - minScore || 1;

    const xScale = (i: number) =>
      PADDING.left + (i / Math.max(1, entries.length - 1)) * INNER_WIDTH;
    const yScale = (score: number) =>
      PADDING.top +
      INNER_HEIGHT -
      ((score - minScore) / scoreRange) * INNER_HEIGHT;
    ...
    return { pathD: linePath, points: pts, trend: getTrend(entries) };
  }, [scoreHistory]);
    ...
    return (
      <Card>
        <CardContent className="flex items-center justify-center py-8 text-muted-foreground">
          <p className="text-xs">
            No score history yet. Complete a profiling session to start
            tracking.
          </p>
        </CardContent>
      </Card>
      ...
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>
              Performance score over {entries.length} session
              {entries.length !== 1 ? "s" : ""}
            </CardDescription>
          </div>
        ...
        >
          {yTicks.map((tick) => {
            const y =
              PADDING.top +
              INNER_HEIGHT -
              ((tick - minScore) / (maxScore - minScore || 1)) * INNER_HEIGHT;
            return (
              <g key={tick}>
                <line
                  x1={PADDING.left}
