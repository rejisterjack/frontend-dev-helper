 ...
 *
 * Ported from react-perf-profiler/apps/ext/src/panel/components/analysis/PerformanceBudgetView.tsx.
 */
...
export function PerformanceBudgetView() {
  ...
  const handleCreate = useCallback(() => {
    ...
    if (newMaxWastedRate)
      rule.maxWastedRenderRate = parseFloat(newMaxWastedRate);
    if (newMaxAvgDuration)
      rule.maxAvgRenderDuration = parseFloat(newMaxAvgDuration);
      ...
      <div className="flex flex-col gap-4 p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            {hasViolations && (
              ...
              >
                {budgetViolations.length} violation
                {budgetViolations.length !== 1 ? "s" : ""}
              </Badge>
            )}
                ...
                >
                  ...
                  <span className="text-muted-foreground">
                    ...
                    <span className="font-mono">
                      {v.actualValue.toFixed(1)}
                    </span>{" "}
                    exceeds <span className="font-mono">{v.threshold}</span>
                  </span>
                </div>
              ))}
            </div>
        ...
        {budgets.length === 0 && !isCreating ? (
          <div className="flex flex-col items-center justify-center py-8 text-muted-foreground gap-2">
            ...
            <p className="text-xs">
              Create a budget to set thresholds for render count, wasted rate,
              and more.
            </p>
          </div>
        ) : (
          <div className="flex flex-col gap-2">
            {budgets.map((budget) => {
              const budgetViolationCount = budgetViolations.filter((v) =>
                budget.rules.some(
                  (r) => r.componentPattern === v.rule.componentPattern,
                ),
              ).length;
                          ...
                          {budget.rules.map((rule, idx) => (
                            ...
                            >
                              ...
                              {rule.maxRenderCount != null &&
                                ` ≤${rule.maxRenderCount} renders`}
                              {rule.maxWastedRenderRate != null &&
                                ` ≤${rule.maxWastedRenderRate}% waste`}
                              {rule.maxAvgRenderDuration != null &&
                                ` ≤${rule.maxAvgRenderDuration}ms avg`}
                            </span>
                          ))}
                        </div>
                      </div>
