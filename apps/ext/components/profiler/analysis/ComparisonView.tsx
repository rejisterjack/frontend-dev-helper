 ...
 *
 * Ported from react-perf-profiler/apps/ext/src/panel/components/analysis/ComparisonView.tsx.
 * Adapted: profile comparison lives in @/lib/profiler/profileComparison.
 */
...
import {
  ...
  type ProfileDiff,
  type ComponentDiff,
} from "@/lib/profiler/profileComparison";
import { Button } from "@/components/ui/button";
import { Card, CardContent } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { ScrollArea } from "@/components/ui/scroll-area";
import { cn } from "@/lib/utils";
import { GitCompare, TrendingUp, TrendingDown, Plus } from "lucide-react";

export function ComparisonView() {
  const commits = useProfilerStore((s) => s.commits);
  const analysisResult = useProfilerStore((s) => s.analysisResult);
  const performanceScore = analysisResult?.performanceScore ?? null;
  const [baselineCommits, setBaselineCommits] = useState<CommitData[] | null>(
    null,
  );
            ...
            {diff.newComponents.length > 0 && (
              <div>
                <h4 className="text-xs font-medium text-muted-foreground mb-2 flex items-center gap-1">
                  <Plus className="size-3" /> New Components (
                ...
                <div className="flex flex-wrap gap-1">
                  {diff.newComponents.map((name) => (
                    <Badge
                      key={name}
                      variant="outline"
                      className="text-[10px] bg-blue-500/10 text-blue-600 border-blue-500/30 dark:text-blue-400"
                    >
                      {name}
                    </Badge>
                  ))}
            ...
            {diff.regressions.length > 0 && (
              <div>
                <h4 className="text-xs font-medium text-red-600 dark:text-red-400 mb-2 flex items-center gap-1">
                  <TrendingDown className="size-3" /> Regressions (
                  {diff.regressions.length})
                </h4>
                <div className="flex flex-col gap-1.5">
                  ...
                  {diff.regressions.length > 20 && (
                    <p className="text-[10px] text-muted-foreground text-center">
                      +{diff.regressions.length - 20} more
                    </p>
                  )}
                </div>
              </div>
            ...
            {diff.improvements.length > 0 && (
              <div>
                <h4 className="text-xs font-medium text-emerald-600 dark:text-emerald-400 mb-2 flex items-center gap-1">
                  <TrendingUp className="size-3" /> Improvements (
                  {diff.improvements.length})
                </h4>
                <div className="flex flex-col gap-1.5">
                  ...
                  {diff.improvements.length > 10 && (
                    <p className="text-[10px] text-muted-foreground text-center">
                      +{diff.improvements.length - 10} more
                    </p>
                  )}
                </div>
              </div>
              ...
              diff.newComponents.length === 0 && (
                <Card>
                  <CardContent className="flex items-center justify-center py-6 text-muted-foreground">
                    <p className="text-sm">
                      No differences detected between baseline and current.
                    </p>
                  </CardContent>
                </Card>
              )}
        ...
        {!hasBaseline && (
          <Card>
            <CardContent className="flex flex-col items-center justify-center py-8 text-muted-foreground gap-2">
              ...
              <p className="text-xs">
                Profile your app, set the current session as baseline, then
                profile again after making changes to compare.
              </p>
            </CardContent>
          </Card>
        )}
  ...
  return (
    ...
    >
      ...
      <span className="text-muted-foreground shrink-0">
        {metricLabels[diff.metric] ?? diff.metric}
      </span>
      <span className="text-muted-foreground tabular-nums shrink-0">
        {formatValue(diff.metric, diff.baselineValue)}
      </span>
