 ...
 * Memoization, Suggestions, and Budgets sub-tabs.
 *
 * Ported from react-perf-profiler/apps/ext/src/panel/components/analysis/AnalysisView.tsx.
 */
...
import { calculateFrameImpact } from "@repo/profiler-analyzer";
...
import MetricsChart from "@/components/profiler/visualizations/MetricsChart";
import { ScoreHistoryChart } from "@/components/profiler/visualizations/ScoreHistoryChart";
...
function OverviewTab({
  ...
  const criticalCount = wastedRenderReports.filter(
    (r) => r.severity === "critical",
  ).length;
  const highCount = wastedRenderReports.filter(
    (r) => r.severity === "high",
  ).length;
          ...
          <CardContent className="flex items-center gap-4 py-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                That&apos;s {frameImpact.droppedFrames} dropped frame
                {frameImpact.droppedFrames !== 1 ? "s" : ""} at 60fps (
                {frameImpact.frameBudgetMs.toFixed(1)}ms budget)
              </p>
            </div>
          </CardContent>
        </Card>
        ...
        for (const commit of commits) {
          ...
          for (const fiber of fibers) {
            ...
            durationsByComponent.set(
              fiber.displayName,
              (durationsByComponent.get(fiber.displayName) ?? 0) +
        ...
        return (
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Total render time across all commits
              </CardDescription>
            </CardHeader>
            <CardContent>
        ...
        <Card>
          ...
          <CardContent className="flex flex-col gap-2">
            {topOpportunities.slice(0, 10).map((opp, idx) => (
              <OpportunityCard
                key={`${opp.componentName}-${opp.type}-${idx}`}
                opportunity={opp}
              />
            ))}
...
/**
 * Derive a ComponentMetrics object for the worst-offending component from
 * the analysis result, so the AISuggestionsPanel has something concrete to
 * analyze. Picks the highest-impact wasted-render report.
 */
function deriveMetricsFromResults(results: AnalysisResult): {
  componentName: string;
  metrics: ComponentMetrics;
} | null {
  if (results.wastedRenderReports.length === 0) return null;
  const worst = [...results.wastedRenderReports].sort(
    (a, b) => b.estimatedSavingsMs - a.estimatedSavingsMs,
  )[0];
  const memo = results.memoReports.find(
    (m) => m.componentName === worst.componentName,
  );
  return {
    componentName: worst.componentName,
    metrics: {
      componentName: worst.componentName,
      renderCount: worst.renderCount,
      wastedRenderCount: worst.wastedRenders,
      wastedRenderRate: worst.wastedRenderRate,
      totalRenderTime: worst.estimatedSavingsMs * worst.renderCount,
      averageRenderTime: worst.estimatedSavingsMs,
      maxRenderTime: worst.estimatedSavingsMs,
      minRenderTime: 0,
      isMemoized: memo?.hasMemo ?? false,
      memoHitRate: memo?.currentHitRate,
      firstSeen: Date.now(),
      lastSeen: Date.now(),
    },
  };
...
function AITab({
  ...
  const derived = deriveMetricsFromResults(results);
  if (!derived) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-muted-foreground">
        <p className="text-sm">No wasted renders detected to analyze.</p>
        <p className="text-xs mt-1">
          AI analysis works best on components with wasted renders.
        </p>
      </div>
    );
  }
    ...
    <div className="flex flex-col h-full overflow-hidden">
      <Tabs defaultValue="overview" className="flex flex-col h-full">
          ...
          <TabsList>
            ...
            <TabsTrigger value="wasted">
              ...
              {analysisResults.wastedRenderReports.length > 0 && (
                <Badge
                  variant="secondary"
                  className="ml-1.5 text-[10px] px-1 py-0 h-4 min-w-4 flex items-center justify-center"
                >
        ...
        <TabsContent value="wasted" className="flex-1 overflow-y-auto">
          <WastedRenderReportView
            reports={analysisResults.wastedRenderReports}
          />
        </TabsContent>
        ...
        <TabsContent value="ai" className="flex-1 overflow-y-auto">
          <AITab results={analysisResults} commits={commits} />
        </TabsContent>
      </Tabs>
    </div>
  );
