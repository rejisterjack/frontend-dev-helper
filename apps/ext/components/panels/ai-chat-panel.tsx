...
function SessionPicker({
  ...
  return (
    <div className="relative">
      ...
      >
        <span className="truncate max-w-[180px]">{active?.title ?? 'New Chat'}</span>
            ...
            <div className="max-h-48 overflow-y-auto p-1.5 flex flex-col gap-0.5">
              {sessions.map((s) => (
                ...
                >
                  ...
                  {s.id === activeSessionId && <Check className="size-3 text-primary shrink-0" />}
                  <button
...
export function AIChatPanel() {
  ...
  useEffect(() => {
    const listener = (message: { type: string; messageId?: string; content?: string; done?: boolean; error?: string }) => {
      ...
      if (message.content) {
        ...
        if (currentMsg) {
          updateMessage(sessionId, msgId, { content: currentMsg.content + message.content });
        }
      }
  ...
  const sendMessage = useCallback(async () => {
    ...
    if (!ai.enabled || !ai.apiKey) {
      addMessage(activeSessionId, {
        ...
        role: 'system',
        content: 'Please configure your AI API key in Settings to use this feature.',
        timestamp: Date.now(),
      });
      return;
    }
    ...
    const currentMessages = useChatSessionsStore
      .getState()
  ...
  }, [input, isLoading, ai.enabled, ai.apiKey, activeSessionId, addMessage, updateMessage, autoTitleSession]);
    ...
    <div className="flex flex-col h-full px-4 pt-3 pb-3">
      ...
      <div className="flex items-center justify-between pb-2 shrink-0">
        <div className="flex items-center gap-2">
          ...
          />
          <span className={`inline-block size-1.5 rounded-full ${ai.enabled ? 'bg-emerald-500' : 'bg-muted-foreground/40'}`} />
        </div>
        <div className="flex items-center gap-0.5">
          <Button variant="ghost" size="icon" className="size-6" onClick={handleNewChat} aria-label="New chat">
            <Plus className="size-3 text-muted-foreground" />
          </Button>
          <Button variant="ghost" size="icon" className="size-6" onClick={handleClear} aria-label="Clear chat">
        ...
        <div className="flex flex-col gap-2">
          {messages.length === 0 && (
            <div className="py-12 text-center">
              ...
              <p className="text-[0.65rem] text-muted-foreground mt-1.5 max-w-[220px] mx-auto leading-relaxed">
                Ask about elements, debug styles, audit accessibility, or get suggestions for the current page.
              </p>
            </div>
          )}
          ...
          {messages.map((msg) => (
            ...
            >
              ...
              >
                {msg.content || (isLoading && msg.id === streamingId ? '' : msg.content)}
                {isLoading && msg.id === streamingId && !msg.content && <StreamingDots />}
              </div>
            </div>
          ))}
