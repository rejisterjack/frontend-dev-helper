...
/**
 * Small input that persists its value to encrypted-at-rest storage
 * (lib/secrets.ts) on blur, never touching the persisted settings store.
 * Shows a placeholder when the secret is set (read once on mount) so the
 * plaintext is never re-rendered.
 */
function SecretInput({
  ...
  field: "aiApiKey" | "githubToken" | "bridgeToken";
  placeholder: string;
  isSetHint: string;
}) {
  ...
  return (
    <div className="flex flex-col gap-1.5 py-2">
      <Input
        ...
        onBlur={() => {
          if (value) {
            void setSecret(field, value.trim());
            setIsSet(true);
            setValue("");
          }
        }}
        placeholder={isSet ? isSetHint : placeholder}
...
function SettingRow({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex items-center justify-between py-2.5">{children}</div>
  );
}
...
export function SettingsPanel() {
  ...
  const isProviderWithModels =
    isFireworks || ai.provider === "openrouter" || isZAI;
  ...
  const handleTestConnection = async () => {
    ...
    try {
      // The API key lives in encrypted-at-rest storage, not the settings store.
      const apiKey = await getSecret("aiApiKey");
      ...
      <div className="flex flex-col px-4 py-3">
        ...
        <div className="mt-2">
          ...
          <SettingRow>
            ...
            <Switch
              checked={showInactiveTools}
              onCheckedChange={(v) => updateSetting("showInactiveTools", v)}
            />
          </SettingRow>
        </div>
        ...
        <div className="mt-2">
          <SettingRow>
            ...
            <Switch
              checked={ai.enabled}
              onCheckedChange={(v) => updateAIConfig({ enabled: v })}
            />
          </SettingRow>
          {ai.enabled && (
            <>
              <SettingRow>
                ...
                <Select
                  value={ai.provider}
                  onValueChange={(v) => {
                    const newProvider = v as
                      | "openrouter"
                      | "ollama"
                      | "fireworks"
                      | "zai"
                      | "custom";
                    const updates: Partial<typeof ai> = {
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="fireworks" className="text-xs">
                      Fireworks
                    </SelectItem>
                    <SelectItem value="zai" className="text-xs">
                      Z.AI
                    </SelectItem>
              ...
              {!isOllama && (
                <SecretInput
                  ...
                  placeholder={
                    isFireworks
                      ? "Get key at fireworks.ai"
                      : isZAI
                        ? "Get key at z.ai"
                        : "sk-..."
                  }
                  isSetHint="(API key saved — type to replace)"
                />
              )}
              ...
              {connectionStatus && (
                <p
                  className={`text-[0.65rem] pt-1.5 ${connectionStatus.success ? "text-emerald-600" : "text-destructive"}`}
                >
            ...
            <>
              <SettingRow>
                ...
                <Input
                  ...
                  value={vscode.port}
                  onChange={(e) =>
                    updateVSCodeConfig({ port: Number(e.target.value) || 9456 })
                  }
                  className="w-20 h-7 text-xs bg-secondary border-0 rounded-md"
                />
              </SettingRow>
              <div className="space-y-1.5">
                <Label className="text-xs">Bridge auth token</Label>
                <SecretInput
                  field="bridgeToken"
                  placeholder="Paste token from VS Code → 'FDH: Copy Bridge Auth Token'"
                  isSetHint="(token saved — type to replace)"
                />
                <p className="text-[0.6rem] text-muted-foreground leading-snug">
                  Required. Run the{" "}
                  <span className="font-mono">FDH: Copy Bridge Auth Token</span>{" "}
                  command in VS Code, then paste the value here and click
                  Reconnect.
                </p>
              </div>
              <SettingRow>
                ...
                <div className="flex items-center gap-1.5">
                  <span
                    className={`inline-block size-1.5 rounded-full ${vscodeConnected ? "bg-emerald-500" : "bg-muted-foreground/40"}`}
                  />
                  <span className="text-[0.65rem] text-muted-foreground">
                    {vscodeConnected ? "Connected" : "Disconnected"}
                  </span>
                </div>
              </SettingRow>
              <div className="pt-1">
        ...
        <div className="mt-2">
          <SettingRow>
            ...
            <Switch
              ...
              onCheckedChange={(v) => updateGitHubConfig({ enabled: v })}
            />
          </SettingRow>
          {github.enabled && (
            <div className="flex flex-col gap-1.5 py-2">
          ...
          <SettingRow>
            ...
            <Switch
              ...
              onCheckedChange={(v) => updateSetting("enableTelemetry", v)}
            />
          </SettingRow>
        </div>
