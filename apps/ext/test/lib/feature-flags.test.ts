...
describe("feature-flags", () => {
  describe("isFeatureEnabled", () => {
    ...
    it("returns false for pro features on free tier", () => {
      ...
      // vscode-bridge is intentionally free — it's the core local dev loop.
      expect(isFeatureEnabled("vscode-bridge", "free")).toBe(true);
    });
  ...
  describe("getRequiredTier", () => {
    it("returns correct tier for pro features", () => {
      ...
      // vscode-bridge is free in the current model — see lib/feature-flags.ts.
      expect(getRequiredTier("vscode-bridge")).toBe("free");
    });
  ...
  describe("getAllFeatureFlags", () => {
    it("returns all feature flags", () => {
      ...
      // Registry grew from 7 → 32 flags; assert against the live source of truth,
      // not a hardcoded count, so this test fails loud when tiers change.
      ...
      expect(flags["vscode-bridge"]).toBe("free");
